...
/* All decorative layers — pointer-events:none so the wheel scrolls through */
.bsc-hero-bg { position:absolute; inset:0; background:url('${SUPABASE}/hero.jpg') center 40%/cover; transform:scale(1.06); transition:transform 8s cubic-bezier(.25,.46,.45,.94); will-change:transform; pointer-events:none; }
...
/* Content — only this layer receives events */
...
.bsc-hero-scroll { position:absolute; bottom:32px; left:50%; transform:translateX(-50%); display:flex; flex-direction:column; align-items:center; gap:7px; z-index:10; cursor:pointer; animation:bscScrollFade .6s ease 1.3s both; pointer-events:auto; }
