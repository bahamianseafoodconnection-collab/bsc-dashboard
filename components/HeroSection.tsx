  ...
  return (
    <>
      <style>{`
        ...
        .bsc-hero-bg { position:absolute; inset:0; background:url('${SUPABASE}/hero.jpg') center 40%/cover; transform:scale(1.06); transition:transform 8s cubic-bezier(.25,.46,.45,.94); will-change:transform; }
        .bsc-hero-bg.on { transform:scale(1.0); }

        .bsc-hero-ov1 { position:absolute; inset:0; background:linear-gradient(108deg, rgba(5,12,24,.95) 0%, rgba(10,21,42,.84) 40%, rgba(15,33,55,.5) 65%, rgba(0,0,0,.15) 100%); }
        .bsc-hero-ov2 { position:absolute; inset:0; background:linear-gradient(to top, rgba(5,12,24,.92) 0%, rgba(5,12,24,.3) 28%, transparent 58%); }
        .bsc-hero-ov3 { position:absolute; inset:0; background:radial-gradient(ellipse 60% 80% at 100% 50%, rgba(212,168,67,.06) 0%, transparent 70%); }

        .bsc-hero-bar { position:absolute; left:0; top:0; bottom:0; width:4px; background:linear-gradient(to bottom, transparent, #d4a843 18%, #f5c842 50%, #d4a843 82%, transparent); opacity:.65; }

        .bsc-hero-inner { position:relative; z-index:10; max-width:1280px; margin:0 auto; padding:100px 6% 0; width:100%; }

        .bsc-hero-eye { display:inline-flex; align-items:center; gap:10px; margin-bottom:22px; opacity:0; transform:translateY(18px); transition:opacity .65s ease .1s, transform .65s ease .1s; }
        ...
        .bsc-hero-h1 { font-family:'Playfair Display',serif; font-weight:900; font-size:clamp(48px,7.8vw,98px); line-height:1.0; color:#fff; margin:0 0 8px; max-width:680px; opacity:0; transform:translateY(28px); transition:opacity .8s ease .25s, transform .8s ease .25s; }
        ...
        .bsc-hero-sep { display:flex; align-items:center; gap:14px; margin-bottom:24px; opacity:0; transform:translateY(18px); transition:opacity .65s ease .42s, transform .65s ease .42s; }
        ...
        .bsc-hero-desc { font-family:'DM Sans',sans-serif; font-size:clamp(14px,1.7vw,17px); line-height:1.78; color:rgba(226,232,240,.72); max-width:440px; margin-bottom:38px; font-weight:300; opacity:0; transform:translateY(18px); transition:opacity .65s ease .52s, transform .65s ease .52s; }
        ...
        .bsc-cta-main { display:inline-flex; align-items:center; gap:8px; padding:15px 34px; background:linear-gradient(130deg,#f5c842 0%,#c8860f 100%); color:#060e1c; font-family:'DM Sans',sans-serif; font-size:13px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; text-decoration:none; border-radius:4px; box-shadow:0 6px 32px rgba(212,160,21,.42), inset 0 1px 0 rgba(255,255,255,.14); transition:transform .2s ease, box-shadow .2s ease, filter .2s ease; position:relative; overflow:hidden; }
        .bsc-cta-main::after { content:''; position:absolute; top:0; left:-100%; width:50%; height:100%; background:linear-gradient(90deg,transparent,rgba(255,255,255,.22),transparent); transition:left .55s ease; }
        ...
        .bsc-cta-main:active { transform:translateY(0); }

        .bsc-cta-ghost { display:inline-flex; align-items:center; gap:8px; padding:14px 26px; background:rgba(255,255,255,.055); color:rgba(255,255,255,.85); font-family:'DM Sans',sans-serif; font-size:13px; font-weight:600; letter-spacing:.08em; text-transform:uppercase; text-decoration:none; border:1.5px solid rgba(255,255,255,.2); border-radius:4px; backdrop-filter:blur(8px); transition:all .22s ease; }
        .bsc-cta-ghost:hover { background:rgba(255,255,255,.1); border-color:rgba(212,168,67,.55); color:#f5c842; transform:translateY(-2px); }

        .bsc-hero-stats { display:flex; gap:0; opacity:0; transform:translateY(14px); transition:opacity .65s ease .78s, transform .65s ease .78s; }
        ...
        .bsc-hero-scroll { position:absolute; bottom:32px; left:50%; transform:translateX(-50%); display:flex; flex-direction:column; align-items:center; gap:7px; z-index:10; cursor:pointer; animation:bscScrollFade .6s ease 1.3s both; }
        @keyframes bscScrollFade { to { opacity:1; } }
        .bsc-hero-scroll { opacity:0; }
