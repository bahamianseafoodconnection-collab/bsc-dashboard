...
// Tell Next.js: never prerender this page. Checkout is always per-user, runtime only.
export const dynamic = 'force-dynamic';

const BASE = 'https://qgcaxkyuhwmpvpbooaqw.supabase.co/storage/v1/object/public/site-images';

// Lazy-init Supabase client. Calling createClient at module scope causes the
// build to fail at static prerender because env vars are not yet loaded.
          ...
          <div>
            ...
            {view === 'done' && (
              <div style={{ backgroundColor: '#fff', borderRadius: 16, padding: '32px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', animation: 'fadeUp 0.4s ease both', textAlign: 'center' }}>
                <div style={{ width: 88, height: 88, borderRadius: '50%', backgroundColor: payMethod === 'card' ? '#d1fae5' : '#dbeafe', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 42, margin: '0 auto 16px', fontWeight: 900, color: payMethod === 'card' ? '#065f46' : '#1a2e4a' }}>
                  {payMethod === 'card' ? 'OK' : 'BOX'}
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 16, maxHeight: 320, overflowY: 'auto' }}>
                  {cart.map(item => (
                    <div key={`${item.source}-${item.id}`} style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                      <div style={{ width: 44, height: 44, borderRadius: 8, backgroundColor: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, color: '#94a3b8', fontWeight: 700, flexShrink: 0, overflow: 'hidden' }}>
                        {item.image_url ? (
                          // eslint-disable-next-line @next/next/no-img-element
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11, color: '#64748b' }}>Qty {item.qty} x BSD ${fmt(item.price)}</div>
                      </div>
