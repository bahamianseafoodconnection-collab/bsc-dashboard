...
function getDutyRate(category: string, productName: string): number {
  ...
  if (cat === "seafood") {
    if (name.includes("grouper") || name.includes("snapper") || name.includes("tuna") || name.includes("mahi") || name.includes("swai") || name.includes("lobster") || name.includes("squid") || name.includes("mussel")) return 0.35;
    if (name.includes("shrimp") || name.includes("salmon") || name.includes("octopus")) return 0;
    return 0.35;
  }
  if (cat === "poultry") {
  ...
  if (cat === "meat") {
    if (name.includes("pork") || name.includes("deer")) return 0.10;
    return 0;
  }
...
export default function SupplierPage() {
      ...
      try {
        ...
        if (user) {
          ...
          if (profile?.role === "control_admin" || profile?.role === "basic_admin" || profile?.role === "manager") {
            ...
            if (spinySupplier) {
              setSupplier(spinySupplier);
              await loadMyProducts(spinySupplier.id);
              setIsSpinyAdmin(true);
            }
            setView("admin");
  ...
  const WhatsAppSidebar = () => (
    <>
      ...
      <button
        ...
        title="Chat with BSC on WhatsApp"
      >
        💬
      </button>
        ...
        <div style={{ position: "fixed", bottom: 90, right: 12, zIndex: 400, width: 340, maxWidth: "calc(100vw - 24px)", backgroundColor: "#0d1f3c", borderRadius: 20, border: "1px solid #1e3a5f", boxShadow: "0 8px 40px rgba(0,0,0,0.6)", overflow: "hidden" }}>
          ...
          <div style={{ background: "linear-gradient(135deg, #25d366, #128c7e)", padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              ...
              <p style={{ margin: "2px 0 0", color: "rgba(255,255,255,0.8)", fontSize: 11 }}>+1 (242) 361-3474 · Admin & Manager</p>
            </div>
            ...
            {waTab === "chat" ? (
              <>
                ...
                <div style={{ backgroundColor: "#060d1f", borderRadius: 12, padding: "12px 14px", marginBottom: 14, border: "1px solid #1e3a5f" }}>
                  ...
                  <p style={{ margin: "2px 0 0", color: "#4a5568", fontSize: 11 }}>Dedrick / Manager on duty</p>
                </div>
                {["Hi BSC! I have a question about my product submission.", "Hi BSC! Can you check the status of my application?", "Hi BSC! I need help updating my product pricing.", "Hi BSC! I have a new product ready to submit."].map(msg => (
                  <a key={msg} href={`https://api.whatsapp.com/send?phone=${BSC_WA_NUMBER}&text=${encodeURIComponent(msg)}`} target="_blank" rel="noopener noreferrer" style={{ display: "block", marginBottom: 8, padding: "10px 12px", backgroundColor: "#111c33", borderRadius: 10, border: "1px solid #1e3a5f", color: "#fff", textDecoration: "none", fontSize: 12, lineHeight: 1.4 }}>
                    💬 {msg}
                  </a>
                ))}
                <a href={`https://api.whatsapp.com/send?phone=${BSC_WA_NUMBER}`} target="_blank" rel="noopener noreferrer" style={{ display: "block", marginTop: 4, padding: "12px", borderRadius: 10, backgroundColor: "#25d366", color: "#fff", fontWeight: "bold", textAlign: "center", textDecoration: "none", fontSize: 14 }}>
                  Open WhatsApp →
                </a>
              </>
            ) : (
      ...
      {showYield && (
        <div style={{ backgroundColor: "#0a1220", border: "1px solid #1e3a5f", borderRadius: "0 0 12px 12px", padding: "16px 14px", marginBottom: 0 }}>
          ...
          <div style={{ backgroundColor: "#060d1f", borderRadius: 10, padding: "12px 14px", marginBottom: 12, border: "1px solid #1e3a5f" }}>
            ...
            <input type="number" placeholder="e.g. 100" value={yieldWeightIn} onChange={e => setYieldWeightIn(e.target.value)} style={{ ...inp, marginBottom: 8 }} />
            <label style={lbl}>Weight Out — After Processing (lbs)</label>
            <input type="number" placeholder="e.g. 35" value={yieldWeightOut} onChange={e => setYieldWeightOut(e.target.value)} style={{ ...inp, marginBottom: 8 }} />
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
                {[
                  ...
                  <div key={x.label} style={{ backgroundColor: "#060d1f", borderRadius: 8, padding: "8px 10px" }}>
                    ...
                    <p style={{ margin: "4px 0 0", color: x.color, fontWeight: "bold", fontSize: 16 }}>{x.value}</p>
                  </div>
                ))}
              </div>
              <p style={{ margin: "0 0 8px", color: "#f5c518", fontWeight: "bold", fontSize: 12 }}>💰 BSC SELLING PRICES (per lb)</p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                {[
                  { label: "Nassau POS 38%",    value: nassauPriceYield,    color: "#60a5fa" },
                  { label: "Andros POS 43%",    value: androsPriceYield,    color: "#a78bfa" },
                  { label: "Online Market 25%", value: onlinePriceYield,    color: "#4ade80" },
                  { label: "Wholesale 12%",     value: wholesalePriceYield, color: "#f5c518" },
                ].map(ch => (
                  <div key={ch.label} style={{ backgroundColor: "#060d1f", borderRadius: 8, padding: "7px 10px" }}>
                    ...
                    <p style={{ margin: "2px 0 0", color: ch.color, fontWeight: "bold", fontSize: 14 }}>${ch.value}/lb</p>
                  </div>
                ))}
              </div>
              <div style={{ marginTop: 10, backgroundColor: "#060d1f", borderRadius: 8, padding: "8px 10px" }}>
                <p style={{ margin: 0, color: "#aaa", fontSize: 11 }}>
                  Based on <b style={{ color: "#fff" }}>{yieldIn} lbs in</b> → <b style={{ color: "#4ade80" }}>{yieldOut} lbs out</b> at <b style={{ color: "#f5c518" }}>${trueCostPerLb}/lb true cost</b>. Submit these prices to BSC for approval before going live.
                </p>
              </div>
            </div>
          )}
          ...
          <div style={{ backgroundColor: "#111c33", borderRadius: 10, padding: "10px 12px", border: "1px solid #1e2d4a" }}>
            ...
            <p style={{ margin: "8px 0 0", color: "#4a5568", fontSize: 10 }}>Reference only. Always use actual processed weight above.</p>
          </div>
        </div>
      )}
    </div>
      ...
      <>
        <div style={{ backgroundColor: currentIsUS ? "#1a0a00" : "#0a1f0a", border: "1px solid " + (currentIsUS ? "#f87171" : "#4ade80"), borderRadius: 10, padding: "10px 14px", marginBottom: 14 }}>
          <p style={{ margin: 0, color: currentIsUS ? "#f87171" : "#4ade80", fontWeight: "bold", fontSize: 13 }}>
            {currentIsUS ? "🇺🇸 US/Florida — Duty + $400 shipping applied" : "🇧🇸 Bahamas Supplier — No duty or shipping"}
          </p>
        </div>
        {isAdmin && (
        ...
        {(prodCategory === "seafood" || prodCategory === "poultry" || prodCategory === "meat" || prodCategory === "general") && (
          <>
            <div style={{ backgroundColor: "#0a1220", borderRadius: 10, padding: "12px 14px", marginBottom: 14, border: "1px solid #1e2d4a" }}>
              ...
              <label style={lbl}>Case Cost Price ($) — Your cost to BSC</label>
            ...
            {foodPricing && isAdmin && (
              <div style={{ backgroundColor: "#0a1220", border: "1px solid #f5c518", borderRadius: 10, padding: "12px 14px", marginBottom: 14 }}>
                ...
                {currentIsUS && <p style={{ margin: "2px 0", color: "#aaa", fontSize: 12 }}>Duty ({(foodPricing.dutyRate * 100).toFixed(0)}%): +${foodPricing.dutyAmount.toFixed(2)}</p>}
                {currentIsUS && <p style={{ margin: "2px 0", color: "#aaa", fontSize: 12 }}>Shipping: +${foodPricing.shippingCost.toFixed(2)}</p>}
                {!currentIsUS && <p style={{ margin: "2px 0", color: "#4ade80", fontSize: 12 }}>✅ No duty or shipping</p>}
        ...
        {prodCategory === "auto" && (
          <div style={{ backgroundColor: "#0a1220", borderRadius: 10, padding: "12px 14px", marginBottom: 14, border: "1px solid #a78bfa44" }}>
            ...
            <p style={{ margin: "0 0 10px", color: "#4a5568", fontSize: 12 }}>Enter your cost price. BSC adds 10% markup + 10% VAT automatically.</p>
            <label style={lbl}>Your Cost Price ($) — REQUIRED</label>
            ...
            {partPricing && (
              <div style={{ backgroundColor: "#060d1f", borderRadius: 8, padding: "10px 12px", marginBottom: 10, border: "1px solid #4ade8044" }}>
                <p style={{ margin: "0 0 6px", color: "#4ade80", fontWeight: "bold", fontSize: 12 }}>📊 CUSTOMER PRICE BREAKDOWN</p>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}><p style={{ margin: 0, color: "#aaa", fontSize: 12 }}>Your Cost</p><p style={{ margin: 0, fontSize: 12 }}>${parseFloat(partSupplierCost).toFixed(2)}</p></div>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}><p style={{ margin: 0, color: "#4ade80", fontSize: 12 }}>BSC Markup (10%)</p><p style={{ margin: 0, color: "#4ade80", fontSize: 12 }}>+${partPricing.markup.toFixed(2)}</p></div>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><p style={{ margin: 0, color: "#a78bfa", fontSize: 12 }}>VAT (10%)</p><p style={{ margin: 0, color: "#a78bfa", fontSize: 12 }}>+${partPricing.vat.toFixed(2)}</p></div>
                <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid #1e3a5f", paddingTop: 6 }}><p style={{ margin: 0, fontWeight: "bold", fontSize: 13 }}>Customer Pays</p><p style={{ margin: 0, color: "#4ade80", fontWeight: "bold", fontSize: 16 }}>${partPricing.customerPrice.toFixed(2)}</p></div>
              </div>
            )}
        ...
        {prodCategory === "vehicle" && (
          <div style={{ backgroundColor: "#0a1220", borderRadius: 10, padding: "12px 14px", marginBottom: 14, border: "1px solid #f5c51844" }}>
            ...
            {vehicleListingType === "sale" ? (
              <>
                <div style={{ backgroundColor: "#060d1f", border: "2px solid #f5c518", borderRadius: 10, padding: "10px 12px", marginBottom: 10 }}>
                  <label style={{ ...lbl, color: "#f5c518" }}>Your Cost Price ($) — REQUIRED</label>
                  <input type="number" placeholder="e.g. 15000.00" value={vehicleSupplierCost} onChange={(e) => setVehicleSupplierCost(e.target.value)} style={{ ...inp, fontSize: 18, fontWeight: "bold", marginBottom: 0, border: "1px solid #f5c518" }} />
                  <p style={{ margin: "6px 0 0", color: "#6b7280", fontSize: 11 }}>BSC adds $650 markup + 10% VAT = customer price</p>
                </div>
                {salePricing && (
                  <div style={{ backgroundColor: "#060d1f", borderRadius: 8, padding: "10px 12px", marginBottom: 10, border: "1px solid #4ade8044" }}>
                    <p style={{ margin: "0 0 6px", color: "#4ade80", fontWeight: "bold", fontSize: 12 }}>📊 CUSTOMER PRICE BREAKDOWN</p>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}><p style={{ margin: 0, color: "#aaa", fontSize: 12 }}>Your Cost</p><p style={{ margin: 0, fontSize: 12 }}>${parseFloat(vehicleSupplierCost).toLocaleString()}</p></div>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}><p style={{ margin: 0, color: "#4ade80", fontSize: 12 }}>BSC Markup (flat)</p><p style={{ margin: 0, color: "#4ade80", fontSize: 12 }}>+$650.00</p></div>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><p style={{ margin: 0, color: "#a78bfa", fontSize: 12 }}>VAT (10%)</p><p style={{ margin: 0, color: "#a78bfa", fontSize: 12 }}>+${salePricing.vat.toFixed(2)}</p></div>
                    <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid #1e3a5f", paddingTop: 6 }}><p style={{ margin: 0, fontWeight: "bold", fontSize: 13 }}>Customer Pays</p><p style={{ margin: 0, color: "#4ade80", fontWeight: "bold", fontSize: 18 }}>${salePricing.customerPrice.toLocaleString()}</p></div>
                    <p style={{ margin: "8px 0 0", color: "#f5c518", fontSize: 12 }}>💰 BSC Profit: $650.00 per car</p>
                  </div>
                )}
              </>
            ) : (
              <>
                <div style={{ backgroundColor: "#060d1f", border: "2px solid #60a5fa", borderRadius: 10, padding: "10px 12px", marginBottom: 10 }}>
                  <label style={{ ...lbl, color: "#60a5fa" }}>Your Daily Rate ($) — REQUIRED</label>
                  <input type="number" placeholder="e.g. 50.00" value={vehicleSupplierDailyRate} onChange={(e) => setVehicleSupplierDailyRate(e.target.value)} style={{ ...inp, fontSize: 18, fontWeight: "bold", marginBottom: 0, border: "1px solid #60a5fa" }} />
                  <p style={{ margin: "6px 0 0", color: "#6b7280", fontSize: 11 }}>BSC adds $10/day markup + 10% VAT = customer rate</p>
                </div>
                {rentalPricing && (
                  <div style={{ backgroundColor: "#060d1f", borderRadius: 8, padding: "10px 12px", marginBottom: 10, border: "1px solid #60a5fa44" }}>
                    <p style={{ margin: "0 0 6px", color: "#60a5fa", fontWeight: "bold", fontSize: 12 }}>📊 RENTAL PRICE BREAKDOWN</p>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      <div style={{ backgroundColor: "#111c33", borderRadius: 8, padding: "8px 10px" }}>
                        <p style={{ margin: 0, color: "#4a5568", fontSize: 9 }}>DAILY CUSTOMER RATE</p>
                        <p style={{ margin: "3px 0 0", color: "#60a5fa", fontWeight: "bold", fontSize: 16 }}>${rentalPricing.dailyCustomer.toFixed(2)}<span style={{ fontSize: 10 }}>/day</span></p>
                        <p style={{ margin: "2px 0 0", color: "#4a5568", fontSize: 10 }}>incl. 10% VAT</p>
                      </div>
                      <div style={{ backgroundColor: "#111c33", borderRadius: 8, padding: "8px 10px" }}>
                        <p style={{ margin: 0, color: "#4a5568", fontSize: 9 }}>WEEKLY CUSTOMER RATE</p>
                        <p style={{ margin: "3px 0 0", color: "#60a5fa", fontWeight: "bold", fontSize: 16 }}>${rentalPricing.weeklyCustomer.toFixed(2)}<span style={{ fontSize: 10 }}>/wk</span></p>
                        <p style={{ margin: "2px 0 0", color: "#4a5568", fontSize: 10 }}>incl. 10% VAT</p>
                      </div>
                    </div>
                    <p style={{ margin: "8px 0 0", color: "#f5c518", fontSize: 12 }}>💰 BSC Profit: $10/day · $70/week</p>
                  </div>
                )}
              </>
            )}
        ...
        <button onClick={handleUpload} disabled={loading} style={{ ...primaryBtn, backgroundColor: loading ? "#555" : "#f5c518", cursor: loading ? "not-allowed" : "pointer" }}>
          {loading ? "Saving..." : isAdmin ? "✅ Save & Go Live" : (editingProduct ? "Save & Re-submit for Approval" : "Submit for Approval")}
        </button>
      </>
    );
  };
    ...
    <div style={pg}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          ...
          <p style={{ margin: "2px 0 0", color: "#4a5568", fontSize: 11 }}>BSC Management · Dedrick Storr</p>
        </div>
        ...
        <div style={{ ...card, borderColor: "#f5c518", background: "linear-gradient(135deg, #1a1200, #2a1e00)", marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              ...
              <p style={{ margin: "2px 0 0", color: "#6b7280", fontSize: 12 }}>{myProducts.length} products · Your supplier account</p>
            </div>
          ...
          {allSuppliers.map(sup => {
            ...
            return (
              <div key={sup.id} style={{ ...card, borderColor: isOwnAccount ? "#f5c518" : sup.status === "pending" ? "#f5c51866" : "#1e2d4a" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      {isOwnAccount && <span style={{ backgroundColor: "#f5c518", color: "#000", borderRadius: 6, padding: "1px 8px", fontSize: 10, fontWeight: "bold" }}>YOU</span>}
                    </div>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: supplierProducts.length > 0 ? 12 : 0, flexWrap: "wrap" as const }}>
                  ...
                  {sup.whatsapp && (
                    <a href={"https://wa.me/" + sup.whatsapp.replace(/\D/g, "")} target="_blank" rel="noopener noreferrer" style={{ padding: "3px 12px", borderRadius: 20, fontSize: 11, backgroundColor: "#0a2010", color: "#4ade80", border: "1px solid #4ade80", textDecoration: "none", fontWeight: "bold" }}>
                  ...
                  <div style={{ marginBottom: sup.status === "pending" ? 12 : 0 }}>
                    ...
                    {supplierProducts.map(prod => (
                      <div key={prod.id} style={{ backgroundColor: "#060d1f", borderRadius: 10, padding: "10px 12px", marginBottom: 8, border: "1px solid #1e2d4a" }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: prod.status === "pending" && !isOwnAccount ? 8 : 0 }}>
                          {[
            ...
            <div key={prod.id} style={card}>
              <div style={{ display: "flex", gap: 12, marginBottom: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {prod.supplier_whatsapp && <a href={"https://wa.me/" + prod.supplier_whatsapp.replace(/\D/g, "")} target="_blank" rel="noopener noreferrer" style={{ color: "#4ade80", fontSize: 11, textDecoration: "none" }}>WhatsApp {prod.supplier_whatsapp}</a>}
                </div>
              </div>
    ...
    <div style={pg}>
      ...
      <div style={{ ...card, background: "linear-gradient(135deg, #0a1220, #0d1a2e)", borderColor: "#f5c518", marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginTop: 10 }}>
          {[{ label: "Nassau", value: "38%", color: "#60a5fa" }, { label: "Andros", value: "43%", color: "#a78bfa" }, { label: "Online", value: "25%", color: "#4ade80" }, { label: "Wholesale", value: "12%", color: "#f5c518" }].map(m => (
      ...
      {myProducts.length === 0
        ...
        : myProducts.map(prod => (
          <div key={prod.id} style={card}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginTop: 8 }}>
                  <div style={{ backgroundColor: "#060d1f", borderRadius: 8, padding: "6px 10px" }}>
                    <p style={{ margin: 0, color: "#4a5568", fontSize: 9 }}>ONLINE MARKET</p>
            ...
            <button onClick={() => startEdit(prod)} style={{ width: "100%", marginTop: 12, padding: "9px", borderRadius: 10, backgroundColor: "#0d1f3c", color: "#f5c518", border: "1px solid #f5c51866", fontWeight: "bold", fontSize: 13, cursor: "pointer" }}>✏️ Edit Product</button>
          </div>
        ))}
    </div>
  );
    ...
    <div style={pg}>
      ...
      {success ? (
        ...
        <>
          ...
          {appLocation === "florida" && <div style={{ backgroundColor: "#1a0a00", border: "1px solid #f87171", borderRadius: 10, padding: "10px 14px", marginBottom: 14 }}><p style={{ margin: 0, color: "#f87171", fontSize: 12 }}>🇺🇸 US suppliers: Bahamas customs duty + $400/pallet shipping applied automatically.</p></div>}
          {appLocation === "bahamas" && <div style={{ backgroundColor: "#0a1f0a", border: "1px solid #4ade80", borderRadius: 10, padding: "10px 14px", marginBottom: 14 }}><p style={{ margin: 0, color: "#4ade80", fontSize: 12 }}>🇧🇸 Bahamas suppliers: No duty or shipping costs applied.</p></div>}
          {error && <p style={{ color: "#f87171", fontSize: 13, backgroundColor: "#2d0000", padding: "10px 12px", borderRadius: 8, marginBottom: 12 }}>{error}</p>}
  ...
  if (view === "login") return (
    <div style={pg}>
      ...
      <div style={{ position: "relative", marginBottom: 12 }}>
        ...
        <button onClick={() => setShowLoginPw(!showLoginPw)} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", fontSize: 18, color: "#6b7280", padding: 0 }}>{showLoginPw ? "🙈" : "👁"}</button>
      </div>
      {error && <p style={{ color: "#f87171", fontSize: 13, backgroundColor: "#2d0000", padding: "10px 12px", borderRadius: 8, marginBottom: 12 }}>{error}</p>}
      ...
      {myProducts.length === 0
        ...
        : myProducts.map(prod => (
          <div key={prod.id} style={card}>
            ...
            <button onClick={() => startEdit(prod)} style={{ width: "100%", marginTop: 12, padding: "9px", borderRadius: 10, backgroundColor: "#0d1f3c", color: "#60a5fa", border: "1px solid #1e3a5f", fontWeight: "bold", fontSize: 13, cursor: "pointer" }}>✏️ Edit Product</button>
          </div>
        ))}
    </div>
  );
  ...
  if (view === "edit") return (
    <div style={pg}>
      ...
      {!isAdmin && <p style={{ color: "#f87171", fontSize: 12, marginBottom: 20 }}>⚠️ Editing will re-submit for BSC approval.</p>}
