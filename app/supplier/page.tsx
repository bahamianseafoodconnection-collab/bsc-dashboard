...
// ── SESSION CHECK — uses getSession() for reliability ──
useEffect(() => {
...
.from("suppliers").select("*").eq("email", ADMIN_EMAIL).single();
if (spinySupplier) {
...
.from("suppliers").select("*").eq("email", user.email).single();
...
country_of_origin: prodOrigin, part_number: prodPartNumber,
vin: prodVin, year_make_model: prodYearMakeModel,
unit_cost: parseFloat(pricing.unitCost.toFixed(2)),
retail_price: pricing.retailPrice, wholesale_price: pricing.wholesalePrice,
duty_rate: pricing.dutyRate, duty_amount: parseFloat(pricing.dutyAmount.toFixed(2)),
shipping_cost: pricing.shippingCost, status,
});
setLoading(false);
if (err) { setError(err.message); return; }
const msg = isAdmin
? `Product added and live! Online: $${pricing.retailPrice} | Nassau: $${pricing.nassauPrice} | Wholesale: $${pricing.wholesalePrice}`
...
if (!editingProduct || !supplier) return;
setError(""); setLoading(true);
const photoUrl = (await uploadPhoto()) || editingProduct.photo_url;
const pricing = calcPricing(prodCaseCost, prodPieces, prodCategory, prodName, isUSSupplier);
const status = isAdmin ? "approved" : "pending";
const { error: err } = await supabase.from("supplier_products").update({
name: prodName, category: prodCategory, sku: prodSku,
supplier_whatsapp: prodWhatsApp || supplier.whatsapp,
photo_url: photoUrl,
case_cost: parseFloat(prodCaseCost) || 0,
case_weight_lbs: parseFloat(prodCaseWeight) || 0,
...
if (err) { setError(err.message); return; }
const msg = isAdmin
? `✅ Product updated and live! Online: $${pricing.retailPrice} | Nassau: $${pricing.nassauPrice} | Wholesale: $${pricing.wholesalePrice}`
...
// ── STYLES ──
const pg: React.CSSProperties = { padding: 18, backgroundColor: "#0a0f1e", minHeight: "100vh", color: "#fff", fontFamily: "sans-serif", paddingBottom: 80, maxWidth: 560, margin: "0 auto" };
const inp: React.CSSProperties = { display: "block", width: "100%", padding: "11px 13px", borderRadius: 10, backgroundColor: "#111c33", color: "#fff", border: "1px solid #1e2d4a", fontSize: 14, marginBottom: 12, boxSizing: "border-box" as const };
...
const pricing = prodCaseCost && prodPieces ? calcPricing(prodCaseCost, prodPieces, prodCategory, prodName, currentIsUS) : null;
return (
<>
...
<p style={{ margin: 0, color: "#4ade80", fontWeight: "bold", fontSize: 13 }}>⚡ Admin — Changes go live instantly. No approval needed.</p>
</div>
)}
...
<input placeholder="e.g. Bahamas, USA" value={prodOrigin} onChange={(e) => setProdOrigin(e.target.value)} style={inp} />
</div>
{pricing && isAdmin && (
...
{currentIsUS && <p style={{ margin: "2px 0", color: "#aaa", fontSize: 12 }}>Duty ({(pricing.dutyRate * 100).toFixed(0)}%): +${pricing.dutyAmount.toFixed(2)}</p>}
{currentIsUS && <p style={{ margin: "2px 0", color: "#aaa", fontSize: 12 }}>Shipping: +${pricing.shippingCost.toFixed(2)}</p>}
{!currentIsUS && <p style={{ margin: "2px 0", color: "#4ade80", fontSize: 12 }}>✅ No duty or shipping</p>}
<p style={{ margin: "4px 0 2px", color: "#aaa", fontSize: 12 }}>Unit Cost: ${pricing.unitCost.toFixed(2)}</p>
<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 10 }}>
{[
{ label: "Nassau POS 38%", value: pricing.nassauPrice, color: "#60a5fa" },
{ label: "Andros POS 43%", value: pricing.androsPrice, color: "#a78bfa" },
{ label: "Online Market 25%", value: pricing.onlinePrice, color: "#4ade80" },
{ label: "Wholesale 12%", value: pricing.wholesalePrice, color: "#f5c518" },
].map(ch => (
...
{loading ? (isEdit ? "Saving..." : "Uploading...") : isAdmin ? (isEdit ? "✅ Save & Go Live" : "✅ Add Product — Go Live") : (isEdit ? "Save & Re-submit for Approval" : "Submit for Approval")}
</button>
</>
);
};
...
<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
<div>
<button onClick={() => setView("admin")} style={{ background: "none", border: "none", color: "#f5c518", fontSize: 14, cursor: "pointer", padding: 0, marginBottom: 6 }}>← Back to Admin</button>
<h2 style={{ margin: 0, color: "#f5c518", fontSize: 20 }}>🦞 Spiny Tails Processing</h2>
<p style={{ margin: "2px 0 0", color: "#4a5568", fontSize: 11 }}>Your products · Changes go live instantly</p>
</div>
</div>
...
{myProducts.length === 0 ? (
<div style={{ ...card, textAlign: "center", padding: 30 }}><p style={{ margin: 0, color: "#4a5568", fontSize: 13 }}>No products yet. Add your first product above.</p></div>
) : myProducts.map(prod => (
<div key={prod.id} style={card}>
...
<p style={{ color: "#4a5568", fontSize: 13, marginBottom: 20 }}>{isAdmin ? "Product goes live instantly. Prices auto-calculated by BSC margins." : "BSC will auto-calculate all channel prices."}</p>
...
{!isAdmin && <p style={{ color: "#f87171", fontSize: 12, marginBottom: 20 }}>⚠️ Editing will re-submit this product for BSC approval.</p>}
