...
// ── MARGINS (admin-controlled — suppliers never see or set these) ──
...
function calcPricing(caseCost: string, pieces: string, category: string, name: string, isUS: boolean) {
  ...
  return {
    ...
    // retail_price stored as online price for market display
...
export default function SupplierPage() {
  ...
  const [isSpinyAdmin, setIsSpinyAdmin]   = useState(false); // admin logged in as Spiny Tails supplier
  ...
  const [adminTab, setAdminTab]           = useState<"suppliers" | "products" | "spiny">("suppliers");
      ...
      try {
        ...
        if (user) {
          ...
          if (
            ...
            setIsAdmin(true);
            await loadAdminData();
            // Also load Spiny Tails as the admin's own supplier record
  ...
  async function loadAdminData() {
    ...
    const { data: products } = await supabase
      .from("supplier_products").select("*").order("created_at", { ascending: false });
  ...
  async function handleApply() {
    ...
    const { error: err } = await supabase.from("suppliers").insert({
      ...
      whatsapp: appWhatsApp, category: appCategory,
      status: "pending", location: appLocation,
    });
    setLoading(false);
    if (err) {
  ...
  async function handleLogin() {
    ...
    if (
      ...
      setIsAdmin(true);
      await loadAdminData();
      // Load Spiny Tails as admin's own supplier record
      const { data: spinySupplier } = await supabase
        .from("suppliers").select("*").eq("email", ADMIN_EMAIL).single();
      if (spinySupplier) {
        setSupplier(spinySupplier);
    ...
    const { data: sup } = await supabase
      .from("suppliers").select("*").eq("email", data.user.email).single();
    setLoading(false);
  ...
  async function uploadPhoto(): Promise<string> {
    ...
    const { error: uploadErr } = await supabase.storage
      .from("product-images").upload(fileName, prodPhoto);
    if (uploadErr) return "";
  ...
  async function handleUpload() {
    ...
    // Admin uploads go live instantly; regular suppliers go to pending
    ...
    const msg = isAdmin
      ...
      : `Product submitted for approval. Online: $${pricing.retailPrice} | Wholesale: $${pricing.wholesalePrice}`;
    setSuccess(msg);
  ...
  async function handleEditSave() {
    ...
    const pricing  = calcPricing(prodCaseCost, prodPieces, prodCategory, prodName, isUSSupplier);
    // Admin edits go live instantly; regular suppliers go back to pending
    const status   = isAdmin ? "approved" : "pending";
  ...
  const secondaryBtn: React.CSSProperties = {
    ...
    border: "1px solid #1e2d4a", fontSize: 14, cursor: "pointer", marginBottom: 10,
  };
  ...
  const ProductForm = ({ isEdit }: { isEdit: boolean }) => {
    ...
    return (
      <>
        ...
        <div
          onClick={() => document.getElementById(isEdit ? "photoInputEdit" : "photoInput")?.click()}
          style={{ width: "100%", height: 140, borderRadius: 12, border: "2px dashed #1e2d4a", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", marginBottom: 14, overflow: "hidden", backgroundColor: "#111c33" }}
        >
          {prodPhotoPreview
            ? <img src={prodPhotoPreview} alt="preview" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            : <div style={{ textAlign: "center" }}><p style={{ margin: 0, fontSize: 28 }}>📷</p><p style={{ margin: "6px 0 0", color: "#4a5568", fontSize: 12 }}>Tap to take photo or upload</p></div>
          }
        </div>
        ...
        {(prodCategory === "seafood" || prodCategory === "poultry" || prodCategory === "meat" || prodCategory === "general") && (
          <>
            ...
            {/* PRICING PREVIEW — admins see all channels, suppliers see nothing */}
            {pricing && isAdmin && (
        ...
        <button
          onClick={isEdit ? handleEditSave : handleUpload}
          disabled={loading}
        ...
        >
          {loading
            ...
            : isAdmin
              ? (isEdit ? "✅ Save & Go Live" : "✅ Add Product — Go Live")
              : (isEdit ? "Save & Re-submit for Approval" : "Submit for Approval")}
        </button>
      </>
    );
  };
    ...
    <div style={pg}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={async () => { await supabase.auth.signOut(); setIsAdmin(false); setIsSpinyAdmin(false); setSupplier(null); setMyProducts([]); setView("home"); }} style={{ background: "none", border: "1px solid #1e2d4a", color: "#f87171", fontSize: 12, cursor: "pointer", padding: "6px 12px", borderRadius: 8 }}>Sign Out</button>
        </div>
      </div>
      ...
      {isSpinyAdmin && (
        <div style={{ ...card, borderColor: "#f5c518", background: "linear-gradient(135deg, #1a1200, #2a1e00)", marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button onClick={() => setView("spiny")} style={{ padding: "9px 16px", borderRadius: 10, backgroundColor: "#f5c518", color: "#000", fontWeight: "bold", border: "none", cursor: "pointer", fontSize: 13 }}>
              Manage →
            </button>
          </div>
        </div>
    ...
    <div style={pg}>
      ...
      <div style={{ ...card, background: "linear-gradient(135deg, #0a1220, #0d1a2e)", borderColor: "#f5c518", marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginTop: 10 }}>
          {[
            { label: "Nassau", value: "38%", color: "#60a5fa" },
            { label: "Andros", value: "43%", color: "#a78bfa" },
            { label: "Online", value: "25%", color: "#4ade80" },
            { label: "Wholesale", value: "12%", color: "#f5c518" },
          ].map(m => (
      ...
      {myProducts.length === 0 ? (
        <div style={{ ...card, textAlign: "center", padding: 30 }}>
          <p style={{ margin: 0, color: "#4a5568", fontSize: 13 }}>No products yet. Add your first product above.</p>
        </div>
      ) : myProducts.map(prod => (
        <div key={prod.id} style={card}>
          ...
          <button onClick={() => startEdit(prod)} style={{ width: "100%", marginTop: 12, padding: "9px", borderRadius: 10, backgroundColor: "#0d1f3c", color: "#f5c518", border: "1px solid #f5c51866", fontWeight: "bold", fontSize: 13, cursor: "pointer" }}>
            ✏️ Edit Product
          </button>
        </div>
      ))}
        ...
        <>
          ...
          {appLocation === "florida" && (
            <div style={{ backgroundColor: "#1a0a00", border: "1px solid #f87171", borderRadius: 10, padding: "10px 14px", marginBottom: 14 }}>
              <p style={{ margin: 0, color: "#f87171", fontSize: 12 }}>🇺🇸 US suppliers: Bahamas customs duty + $400/pallet shipping applied automatically.</p>
            </div>
          )}
          {appLocation === "bahamas" && (
            <div style={{ backgroundColor: "#0a1f0a", border: "1px solid #4ade80", borderRadius: 10, padding: "10px 14px", marginBottom: 14 }}>
  ...
  if (view === "upload") return (
    <div style={pg}>
      ...
      <p style={{ color: "#4a5568", fontSize: 13, marginBottom: 20 }}>
        {isAdmin ? "Product goes live instantly. Prices auto-calculated by BSC margins." : "BSC will auto-calculate all channel prices."}
      </p>
      {success && (
        <div style={{ backgroundColor: "#0a1f0a", border: "1px solid #4ade80", borderRadius: 10, padding: "12px 14px", marginBottom: 16 }}>
