...
type View = "home" | "apply" | "login" | "portal" | "upload";
...
const [prodWhatsApp, setProdWhatsApp] = useState(supplier?.whatsapp || "");
...
setSuccess("Application submitted! Dedrick will review and contact you on WhatsApp.");
}
...
setSuccess(
"Product submitted for approval. Retail: $" +
retailPrice +
" | Wholesale: $" +
wholesalePrice
);
...
Apply to become a BSC supplier. Dedrick will review your application.
</p>
<button onClick={() => setView("apply")} style={primaryBtn}>
Apply Now
</button>
</div>
...
<button onClick={() => setView("login")} style={secondaryBtn}>
Supplier Login
</button>
</div>
</div>
...
Fill out the form below. Dedrick will review and contact you on WhatsApp.
</p>

{success ? (
<div style={{ backgroundColor: "#0a1f0a", border: "1px solid #4ade80", borderRadius: 12, padding: 20, textAlign: "center" }}>
<p style={{ color: "#4ade80", fontSize: 15, margin: "0 0 16px" }}>
{success}
</p>
<button onClick={() => { setView("home"); setSuccess(""); }} style={secondaryBtn}>
Back to Home
</button>
</div>
) : (
...
// PORTAL — product list + upload button
if (view === "portal") return (
<div style={pg}>
<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
<div>
<h2 style={{ margin: 0, color: "#f5c518", fontSize: 18 }}>
My Products
</h2>
<p style={{ margin: "2px 0 0", color: "#4a5568", fontSize: 12 }}>
{supplier?.company_name}
</p>
</div>
<button
onClick={() => supabase.auth.signOut().then(() => { setSupplier(null); setView("home"); })}
...
Products you upload appear here after approval
</p>
</div>
</div>
);
...
<p style={{ color: "#4a5568", fontSize: 13, marginBottom: 20 }}>
Fill in product details. BSC will calculate retail and wholesale prices automatically.
</p>
...
<p style={{ margin: "6px 0 0", color: "#4a5568", fontSize: 12 }}>
...
<p style={{ margin: "0 0 10px", color: "#60a5fa", fontSize: 12, fontWeight: "bold" }}>
FOOD / SEAFOOD DETAILS
</p>
...
<p style={{ margin: "0 0 8px", color: "#4ade80", fontSize: 12, fontWeight: "bold" }}>
ESTIMATED PRICING
</p>
{(() => {
...
Duty ({(duty * 100).toFixed(0)}%): +${dutyAmt.toFixed(2)}
</p>
<p style={{ margin: "2px 0", color: "#aaa", fontSize: 12 }}>
Shipping: +$400.00
</p>
<p style={{ margin: "2px 0", color: "#aaa", fontSize: 12 }}>
Unit Cost: ${unit.toFixed(2)}
</p>
<p style={{ margin: "6px 0 2px", color: "#4ade80", fontWeight: "bold", fontSize: 13 }}>
Retail Price: ${(unit * 1.25).toFixed(2)}
</p>
<p style={{ margin: "2px 0", color: "#f5c518", fontWeight: "bold", fontSize: 13 }}>
Wholesale Price: ${(unit * 1.12).toFixed(2)}
</p>
</>
);
})()}
...
{prodCategory === "auto" && (
<div style={{ backgroundColor: "#0a1220", borderRadius: 10, padding: "12px 14px", marginBottom: 14, border: "1px solid #1e2d4a" }}>
<p style={{ margin: "0 0 10px", color: "#a78bfa", fontSize: 12, fontWeight: "bold" }}>
AUTO PART DETAILS
</p>
...
<label style={lbl}>Case Cost / Unit Price ($)</label>
<input type="number" placeholder="0.00" value={prodCaseCost} onChange={(e) => setProdCaseCost(e.target.value)} style={inp} />
</div>
)}
...
<div style={{ backgroundColor: "#0a1220", borderRadius: 10, padding: "12px 14px", marginBottom: 14, border: "1px solid #1e2d4a" }}>
<p style={{ margin: "0 0 10px", color: "#f87171", fontSize: 12, fontWeight: "bold" }}>
VEHICLE DETAILS
</p>
