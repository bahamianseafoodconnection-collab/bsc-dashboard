...
export default function Dashboard() {
  ...
  const [aiMessages, setAiMessages]         = useState<AIMessage[]>([
    { role: 'ai', text: "Hi Dedrick! I'm your BSC AI assistant. I know your live business data — Spiny Tails Processing, BSC Marketplace Nassau, and Ceta's Variety Store in Andros. Ask me anything." }
  ]);
...
3. Spiny Tails Processing Plant — Firetrial Road, Nassau
Freezer stock: ${TOTAL_LBS.toLocaleString()} lbs, 30,000lb capacity.
Revenue: Nassau $${nassauRevenue.toFixed(2)} | Andros $${androsRevenue.toFixed(2)} | Online $${marketRevenue.toFixed(2)}
...
Be concise and actionable.`;
    ...
    <>
        ...
        <>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
              <p style={{ margin: 0, color: '#f5c518', fontWeight: 'bold', fontSize: 14 }}>Recent Sales</p>
      ...
      {section === 'orders' && (
        <div style={{ textAlign: 'center', paddingTop: 60 }}>
          ...
          <Link href="/orders" style={{ display: 'inline-block', background: 'linear-gradient(135deg, #f5c518, #e6b800)', color: '#000', fontWeight: 'bold', padding: '16px 40px', borderRadius: 14, textDecoration: 'none', fontSize: 17 }}>
      ...
      {section === 'market' && (
        <div style={{ textAlign: 'center', paddingTop: 60 }}>
          ...
          <br />
          <Link href="/market" target="_blank" style={{ display: 'inline-block', background: 'linear-gradient(135deg, #f5c518, #e6b800)', color: '#000', fontWeight: 'bold', padding: '16px 40px', borderRadius: 14, textDecoration: 'none', fontSize: 17 }}>
            Open Marketplace →
          </Link>
        </div>
      )}
  ...
  // ── POLISHED BOTTOM NAV — shown on both mobile and desktop (right side of header on desktop is sidebar) ──
  const BottomNav = () => (
    <div style={{
      position: 'fixed', bottom: 0, left: isMobile ? 0 : 260, right: 0, zIndex: 100,
      ...
      {/* Label */}
      <p style={{ margin: '0 0 6px', textAlign: 'center', color: 'rgba(245,197,24,0.3)', fontSize: 8, letterSpacing: 3, fontWeight: 'bold' }}>
        BSC QUICK CONTROL
      </p>
      <div style={{
            ...
            <button
              ...
              style={{
                ...
                background: isActive
                  ...
                  : 'rgba(13,31,60,0.6)',
                boxShadow: isActive
                  ? '0 0 18px rgba(245,197,24,0.2), inset 0 1px 0 rgba(245,197,24,0.15)'
                  : '0 1px 4px rgba(0,0,0,0.3)',
                outline: isActive ? '1px solid rgba(245,197,24,0.3)' : '1px solid rgba(255,255,255,0.04)',
                transition: 'all 0.18s ease',
              }}
            >
              <span style={{ fontSize: isMobile ? 20 : 18 }}>{item.icon}</span>
              <span style={{
                fontSize: 8, letterSpacing: 0.8, fontWeight: isActive ? 'bold' : '500',
              ...
              {/* active dot */}
              {isActive && (
                <div style={{ width: 4, height: 4, borderRadius: '50%', backgroundColor: '#f5c518', marginTop: 2, boxShadow: '0 0 6px #f5c518' }} />
              )}
              {/* pending badge */}
              {hasBadge && (
                <span style={{
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#060d1f', color: '#fff', fontFamily: "'Inter', -apple-system, sans-serif" }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        {/* BOTTOM NAV — always visible on dashboard */}
        <BottomNav />
      </div>
    </div>
  );
