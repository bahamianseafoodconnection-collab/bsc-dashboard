...
export default function AndrosPOSPage() {
  ...
  function addPinDigit(d: string) { if (pinInput.length < 8) setPinInput((p) => p + d); }
    ...
    return (
      <div style={{ minHeight: '100vh', backgroundColor: '#1a0a2e', fontFamily: 'system-ui, -apple-system, sans-serif', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        <div style={{ width: '100%', maxWidth: '340px' }}>
          ...
          <div style={{ backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: '16px', padding: '20px', marginBottom: '14px', textAlign: 'center' }}>
            <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '14px' }}>Enter Staff PIN</div>
            <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginBottom: '10px' }}>
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} style={{ width: '13px', height: '13px', borderRadius: '50%', backgroundColor: i < pinInput.length ? '#7c3aed' : 'rgba(255,255,255,0.15)' }} />
              ))}
            </div>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ backgroundColor: '#fff', borderBottom: '1px solid #ebebeb', padding: '10px 16px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', overflowX: 'auto' }}>
              {CATEGORIES.map((cat) => (
                <button key={cat} onClick={() => setCategory(cat)} style={{ padding: '6px 14px', borderRadius: '20px', border: 'none', backgroundColor: category === cat ? '#7c3aed' : '#f0f0f0', color: category === cat ? '#fff' : '#555', fontSize: '12px', fontWeight: category === cat ? 800 : 500, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                  {cat}
                </button>
              ))}
            </div>
        ...
        <div style={{ width: '320px', backgroundColor: '#fff', borderLeft: '1px solid #ede9fe', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
          ...
          <div style={{ padding: '10px 16px', borderBottom: '1px solid #f5f3ff' }}>
            <input type="text" placeholder="Customer name (optional)" value={customerName} onChange={(e) => setCustomerName(e.target.value)} style={{ width: '100%', padding: '8px 12px', borderRadius: '8px', border: '1.5px solid #ede9fe', fontSize: '13px', outline: 'none', boxSizing: 'border-box' }} />
          </div>
        ...
        <>
          ...
          <div style={{ position: 'fixed', inset: '20px', maxWidth: '400px', margin: '0 auto', backgroundColor: '#fff', borderRadius: '20px', zIndex: 61, overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ padding: '24px' }}>
              ...
              <div style={{ marginTop: '16px', padding: '14px', backgroundColor: '#f5f0ff', borderRadius: '12px', marginBottom: '16px' }}>
