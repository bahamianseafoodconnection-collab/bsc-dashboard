...
export default function AndrosPOSPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      supabase.from('profiles').select('role').eq('id', user.id).single().then(({ data }) => {
        if (data?.role === 'control_admin') {
          setIsControlAdmin(true);
          // Auto-bypass PIN for control admin
          setScreen('shop');
        }
      });
    });
  ...
  const handlePinSubmit = () => {
    if (pinInput.toUpperCase() === ANDROS_PIN) {
      setScreen('shop'); setPinError('');
    } else {
      setPinError('Incorrect PIN. Contact BSC management.'); setPinInput('');
    }
  };
  ...
  const handleCustomerQuery = useCallback((val: string) => {
    ...
    if (isPhone) setCustomerPhone(val); else setCustomerName(val);
  ...
  const selectCustomer = useCallback((c: Customer) => {
    ...
    setCustomerQuery(c.name); setSuggestions([]);
  }, []);
  ...
  const handleCompleteSale = async () => {
    ...
    const sale = {
      ...
      customerPhone,
      items: cart.map(item => ({ productId: item.id, productName: item.name, price: item.price, qty: item.qty, supplierName: item.supplierName })),
      total: cartTotal,
    };
...
<div class="receipt">
  ...
  const CustomerInput = () => (
    <div style={card}>
      ...
      <input placeholder="Type name or phone number..." value={customerQuery} onChange={(e) => handleCustomerQuery(e.target.value)} autoComplete="off" autoCorrect="off" spellCheck={false} style={{ ...inp, marginBottom: suggestions.length > 0 ? 2 : 8 }} />
      ...
      {customerName && customerPhone ? (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#0a1f0a', borderRadius: 10, padding: '10px 12px', border: '1px solid #4ade80' }}>
          ...
          <button onClick={() => { setCustomerName(''); setCustomerPhone(''); setCustomerQuery(''); setSuggestions([]); }} style={{ background: 'none', border: 'none', color: '#6b7280', cursor: 'pointer', fontSize: 12 }}>Change</button>
        </div>
      ) : (
        <>
    ...
    <div style={{ ...pg, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
      <div style={{ width: '100%', maxWidth: 380 }}>
        ...
        <div style={card}>
          ...
          <input type="password" placeholder="Enter PIN to access POS..." value={pinInput} onChange={(e) => setPinInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handlePinSubmit()} autoComplete="off" style={{ ...inp, fontSize: 20, textAlign: 'center', letterSpacing: 4 }} />
          {pinError && <p style={{ color: '#f87171', fontSize: 13, backgroundColor: '#2d0000', padding: '10px 12px', borderRadius: 8, marginBottom: 10, textAlign: 'center' }}>{pinError}</p>}
          <button onClick={handlePinSubmit} style={{ ...primaryBtn, marginBottom: 0 }}>Access Andros POS →</button>
        </div>
        <button onClick={() => router.push('/')} style={{ ...secondaryBtn, marginTop: 8 }}>← Back to Dashboard</button>
      </div>
    </div>
  );
          ...
          <div key={product.id} style={card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
              ...
              {inCart ? (
                ...
                <button onClick={() => addToCart(product)} style={{ padding: '10px 18px', borderRadius: 10, backgroundColor: '#7c3aed', color: '#fff', fontWeight: 'bold', border: 'none', cursor: 'pointer', fontSize: 14, flexShrink: 0 }}>Add</button>
              )}
            </div>
          </div>
        );
        ...
        <div key={item.id} style={card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <p style={{ margin: '3px 0 0', color: '#aaa', fontSize: 13 }}>{item.qty} × ${item.price.toFixed(2)} = <span style={{ color: '#a78bfa', fontWeight: 'bold' }}>${(item.qty * item.price).toFixed(2)}</span></p>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
    ...
    <div style={pg}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
        {[{ method: 'cash' as PaymentMethod, icon: '💵', label: 'Cash' }, { method: 'card' as PaymentMethod, icon: '💳', label: 'Card' }].map(opt => (
          <button key={opt.label} onClick={() => setPaymentMethod(opt.method)} style={{ padding: '20px 16px', borderRadius: 14, backgroundColor: paymentMethod === opt.method ? '#7c3aed' : '#1a0d2e', color: '#fff', border: paymentMethod === opt.method ? 'none' : '1px solid #4c1d95', fontWeight: 'bold', fontSize: 16, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
        ...
        <div style={card}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' as const }}>
            {[cartTotal, Math.ceil(cartTotal / 5) * 5, Math.ceil(cartTotal / 10) * 10, Math.ceil(cartTotal / 20) * 20, 50, 100]
              ...
              <button key={amt} onClick={() => setCashGiven(amt.toFixed(2))} style={{ padding: '8px 14px', borderRadius: 8, backgroundColor: cashNum === amt ? '#7c3aed' : '#4c1d95', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 'bold', fontSize: 13 }}>${amt.toFixed(0)}</button>
            ))}
          </div>
      ...
      <button onClick={handleCompleteSale} disabled={processing || !paymentMethod || (paymentMethod === 'cash' && cashNum < cartTotal)} style={{ ...primaryBtn, marginTop: 16, backgroundColor: processing ? '#555' : (!paymentMethod || (paymentMethod === 'cash' && cashNum < cartTotal)) ? '#2a2a2a' : '#7c3aed', cursor: (processing || !paymentMethod) ? 'not-allowed' : 'pointer' }}>
  ...
  if (screen === 'complete' && completedInvoice) return (
    <div style={pg}>
      ...
      <div style={{ backgroundColor: '#fff', color: '#111', borderRadius: 14, padding: 20, marginBottom: 20, fontFamily: 'monospace' }}>
        ...
        <p style={{ margin: '12px 0 0', color: '#999', fontSize: 10, textAlign: 'center' as const }}>Thank you for shopping at Ceta's Variety Store! 🐟</p>
      </div>
