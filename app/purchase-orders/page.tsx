...
type POItem = { name: string; cases: number; unitDescription: string; totalLbs: number; costPerCase: number; totalCost: number; };
type PurchaseOrder = { id: string; supplier_name: string; invoice_photo_url: string; ai_summary: string; items: POItem[]; total_cost: number; retail_physical: number; retail_online: number; wholesale_physical: number; wholesale_online: number; status: string; allocated_by: string; created_at: string; };
type Allocation = { retail_physical: number; retail_online: number; wholesale_physical: number; wholesale_online: number; };
...
export default function PurchaseOrdersPage() {
  ...
  const [manualItems, setManualItems] = useState<POItem[]>([{ name: '', cases: 0, unitDescription: '', totalLbs: 0, costPerCase: 0, totalCost: 0 }]);
  const [useManual, setUseManual] = useState(false);
  const [allocation, setAllocation] = useState<Allocation>({ retail_physical: 0, retail_online: 0, wholesale_physical: 0, wholesale_online: 0 });
      ...
      reader.onload = async (e) => {
        ...
        try {
          ...
          if (parsed.items?.length > 0) {
            ...
            setAllocation({ retail_physical: Math.round(totalCases * 0.4), retail_online: Math.round(totalCases * 0.2), wholesale_physical: Math.round(totalCases * 0.3), wholesale_online: Math.round(totalCases * 0.1) });
          }
        } catch { setAiSummary('Could not fully parse invoice. Please verify items manually.'); setUseManual(true); }
        setAiLoading(false);
      };
  ...
  function updateManualItem(index: number, field: keyof POItem, value: string | number) {
    setManualItems(prev => {
      ...
      if (field === 'cases' || field === 'costPerCase') updated[index].totalCost = updated[index].cases * updated[index].costPerCase;
      return updated;
    });
  }

  function addManualItem() { setManualItems(prev => [...prev, { name: '', cases: 0, unitDescription: '', totalLbs: 0, costPerCase: 0, totalCost: 0 }]); }
  function removeManualItem(index: number) { setManualItems(prev => prev.filter((_, i) => i !== index)); }
  ...
  async function handleSubmit() {
    ...
    const { error: insertErr } = await supabase.from('purchase_orders').insert({
      supplier_name: supplierName, invoice_photo_url: photoUrl,
      ai_summary: aiSummary || 'Manual entry', items: JSON.stringify(activeItems),
      total_cost: totalCost, retail_physical: allocation.retail_physical,
      retail_online: allocation.retail_online, wholesale_physical: allocation.wholesale_physical,
      wholesale_online: allocation.wholesale_online, status: 'allocated', allocated_by: allocatedBy,
    });
    setProcessing(false);
    ...
    await loadOrders();
    setTimeout(() => {
      setSuccess(''); setScreen('list'); setPhoto(null); setPhotoPreview('');
  ...
  const pg: React.CSSProperties = { padding: 18, backgroundColor: '#060d1f', minHeight: '100vh', color: '#fff', fontFamily: 'sans-serif', paddingBottom: 80, maxWidth: 620, margin: '0 auto' };
  const card: React.CSSProperties = { backgroundColor: '#0d1f3c', borderRadius: 14, padding: '16px 18px', border: '1px solid #1e3a5f', marginBottom: 14 };
  ...
  const secondaryBtn: React.CSSProperties = { width: '100%', padding: '12px', borderRadius: 12, backgroundColor: 'transparent', color: '#6b7280', border: '1px solid #1e3a5f', fontSize: 14, cursor: 'pointer', marginBottom: 10 };
  const statusColor = (status: string) => ({ pending: '#f5c518', allocated: '#4ade80', processed: '#60a5fa' })[status] || '#aaa';
                ...
                {items.map((item, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid #1e3a5f' }}>
                    <div>
                      ...
                      <p style={{ margin: 0, color: '#4a5568', fontSize: 11 }}>{item.cases} cases {item.unitDescription && '· ' + item.unitDescription}{item.totalLbs > 0 && ' · ' + item.totalLbs + ' lbs'}</p>
                    </div>
    ...
    <div style={pg}>
      ...
      <div style={card}>
        ...
        <div onClick={() => document.getElementById('poPhotoInput')?.click()} style={{ width: '100%', height: photoPreview ? 200 : 130, borderRadius: 12, border: '2px dashed ' + (photoPreview ? '#4ade80' : '#1e3a5f'), display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', overflow: 'hidden', backgroundColor: '#060d1f', marginBottom: 10 }}>
          {photoPreview ? <img src={photoPreview} alt="Invoice" style={{ width: '100%', height: '100%', objectFit: 'contain' }} /> : (
            ...
            <div key={i} style={{ backgroundColor: '#060d1f', borderRadius: 10, padding: '10px 12px', marginBottom: 8, border: '1px solid #1e3a5f' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ margin: '2px 0 0', color: '#4a5568', fontSize: 11 }}>{item.cases} cases {item.unitDescription && '· ' + item.unitDescription}{item.totalLbs > 0 && ' · ' + item.totalLbs + ' lbs'}</p>
                </div>
        ...
        <div style={card}>
          ...
          {manualItems.map((item, i) => (
            <div key={i} style={{ backgroundColor: '#060d1f', borderRadius: 12, padding: '12px 14px', marginBottom: 10, border: '1px solid #1e3a5f' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <div><label style={lbl}>Cases</label><input type="number" placeholder="0" value={item.cases || ''} onChange={(e) => updateManualItem(i, 'cases', parseFloat(e.target.value) || 0)} style={{ ...inp, marginBottom: 0 }} /></div>
                <div><label style={lbl}>Cost Per Case ($)</label><input type="number" placeholder="0.00" value={item.costPerCase || ''} onChange={(e) => updateManualItem(i, 'costPerCase', parseFloat(e.target.value) || 0)} style={{ ...inp, marginBottom: 0 }} /></div>
                <div><label style={lbl}>Unit (e.g. 33lb case)</label><input placeholder="33lb case" value={item.unitDescription} onChange={(e) => updateManualItem(i, 'unitDescription', e.target.value)} style={{ ...inp, marginBottom: 0 }} /></div>
                <div><label style={lbl}>Total lbs</label><input type="number" placeholder="0" value={item.totalLbs || ''} onChange={(e) => updateManualItem(i, 'totalLbs', parseFloat(e.target.value) || 0)} style={{ ...inp, marginBottom: 0 }} /></div>
              </div>
              {item.cases > 0 && item.costPerCase > 0 && <p style={{ margin: '8px 0 0', color: '#4ade80', fontWeight: 'bold', fontSize: 13 }}>Total: ${(item.cases * item.costPerCase).toFixed(2)}</p>}
            </div>
          ))}
      ...
      {error && <p style={{ color: '#f87171', fontSize: 13, backgroundColor: '#2d0000', padding: '10px 14px', borderRadius: 10, marginBottom: 12 }}>{error}</p>}

      <button onClick={handleSubmit} disabled={processing || !supplierName || (aiItems.length === 0 && !manualItems[0]?.name)} style={{ ...primaryBtn, backgroundColor: processing ? '#555' : '#f5c518', cursor: processing ? 'not-allowed' : 'pointer' }}>
