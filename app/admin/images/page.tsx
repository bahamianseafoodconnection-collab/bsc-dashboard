...
function matchProductsToFilename(filename: string, products: Product[]): Product[] {
  ...
  const scored = products.map(p => {
    ...
    if (clean.includes('salmon') && pname.includes('salmon')) score += 20;
...
export default function AdminImagesPage() {
  ...
  async function handleZipUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const [name, zipEntry] of imageFiles) {
        ...
        newMatches.push({ filename, blob, previewUrl, matchedProducts, status: 'pending' });
      }
  ...
  async function uploadOne(index: number): Promise<void> {
    ...
    try {
      ...
      setMatches(prev => prev.map((m, i) => i === index ? { ...m, status: 'done', uploadedUrl: publicUrl } : m));
      setDoneCount(c => c + 1);
      setProducts(prev => prev.map(p =>
        match.matchedProducts.find(mp => mp.id === p.id) ? { ...p, image_url: publicUrl } : p
      ));
    } catch (err: any) {
      setMatches(prev => prev.map((m, i) => i === index ? { ...m, status: 'error', error: err.message } : m));
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="p-4">
                ...
                <button
                  ...
                  {zipProcessing ? (
                    ...
                    <div>
                      ...
                      <p className="text-white font-bold text-lg">Upload Product Images Zip</p>
              ...
              <div className="space-y-3">
                <div className="bg-gray-900 rounded-xl p-4 border border-gray-700">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <p className="font-bold text-white">{matches.length} images found</p>
                      <p className="text-xs text-gray-400">{pendingMatches} ready · {doneCount} done</p>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => { setMatches([]); if (zipRef.current) zipRef.current.value = ''; }}
                        className="px-3 py-2 rounded-xl text-xs font-bold bg-gray-800 text-gray-400">✕ Clear</button>
                      <button onClick={uploadAll} disabled={uploadingAll || pendingMatches === 0}
                  ...
                  {uploadingAll && (
                    <div className="h-2 w-full rounded-full bg-gray-800">
                      <div className="h-2 rounded-full transition-all"
                        style={{ width: `${(doneCount / Math.max(pendingMatches, 1)) * 100}%`, backgroundColor: '#f5c518' }} />
                    </div>
                  )}
                </div>

                {matches.map((match, index) => (
                  <div key={match.filename} className="bg-gray-900 rounded-xl border overflow-hidden"
                    ...
                    <div className="flex gap-3 p-3">
                      <div className="relative shrink-0 w-20 h-20 rounded-xl overflow-hidden bg-gray-800">
                        <img src={match.previewUrl} alt={match.filename} className="w-full h-full object-cover" />
                        {match.status === 'done' && <div className="absolute inset-0 bg-green-600/80 flex items-center justify-center text-white text-2xl">✓</div>}
                        {match.status === 'uploading' && <div className="absolute inset-0 bg-black/60 flex items-center justify-center text-white animate-pulse">⏳</div>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-mono text-gray-400 truncate">{match.filename}</p>
                        {match.matchedProducts.length > 0 ? (
                          <div className="mt-1 space-y-0.5">
                            <p className="text-[10px] text-gray-500 uppercase tracking-wide">Matches {match.matchedProducts.length} product{match.matchedProducts.length > 1 ? 's' : ''}:</p>
                            {match.matchedProducts.slice(0, 4).map(p => (
                              <div key={p.id} className="text-xs font-semibold text-white truncate">· {p.name}</div>
                            ))}
                            {match.matchedProducts.length > 4 && <p className="text-[10px] text-gray-500">+{match.matchedProducts.length - 4} more</p>}
                          </div>
                        ) : (
                          <p className="text-xs text-yellow-500 mt-1">⚠ No product match found</p>
                        )}
                        {match.status === 'error' && <p className="text-xs text-red-400 mt-1">✕ {match.error}</p>}
                      </div>
                      <div className="shrink-0 flex items-center">
                        {match.status === 'pending' && match.matchedProducts.length > 0 && (
                          <button onClick={() => uploadOne(index)} disabled={uploadingAll}
                            className="px-3 py-2 rounded-xl text-xs font-bold disabled:opacity-40"
                            style={{ backgroundColor: '#f5c518', color: '#060d1f' }}>⬆</button>
                        )}
                        {match.status === 'done' && <span className="text-green-400 text-xs font-bold">Done ✓</span>}
                        {match.status === 'uploading' && <span className="text-yellow-400 text-xs animate-pulse">Uploading…</span>}
                        {match.matchedProducts.length === 0 && <span className="text-gray-600 text-xs">Skip</span>}
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {mode === 'single' && (
          <div className="space-y-3">
            <p className="text-xs text-gray-400 bg-gray-900 rounded-xl px-4 py-3 border border-gray-700">
              📱 Tap <strong className="text-white">📷 Upload</strong> on any product to choose from your <strong className="text-white">camera, photo gallery, or files app</strong>
            </p>
                ...
                {filteredProducts.map(product => (
                  <div key={product.id}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {product.image_url
                        ? <p className="text-[10px] text-green-400 mt-0.5">✓ Image set</p>
                        : <p className="text-[10px] text-yellow-400 mt-0.5">⚠ No image</p>}
                    </div>
                    <div className="shrink-0">
                      {/* NO capture attribute — lets user choose camera, gallery, or files */}
                      <input
