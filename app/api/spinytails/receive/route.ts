...
//
// Receiving Station device endpoint (Phase 2). Captures an incoming lot of any
// species, generates the permanent species-prefixed batch number
// (CON-/LOB-/SNP-YYYYMMDD-NNN), validates CCP-1 (temperature + sulfite) against
// the species' configured limits, and writes the lot + intake records.
//
// Server-authoritative: batch number, daily sequence, employee, and the CCP-1
// pass/fail are all stamped here — the device cannot forge them.
//
// Body: { vessel_id, species_code, receipt_date?, product_name, num_bags,
//   total_weight_lbs, weight_per_bag_lbs, product_state ('fresh'|'frozen'),
//   product_grade, product_condition, core_temp_f, fishing_area, fishing_method,
//   trip_start_location, trip_end_location, fishing_date_start, fishing_date_end,
//   qc_results, harvest_photos, device_id }
// Resp: { ok, batch_number, lot_id, qc_pass, ccp_warnings }
...
export async function POST(req: NextRequest) {
  ...
  const { data: sp } = await admin.from('spinytails_species').select('code, ccp_limits').eq('code', species).maybeSingle<{ code: string; ccp_limits: Record<string, unknown> }>();
  ...
  // ── Batch number (species-prefixed) + daily sequence ──────────────────────
  ...
  // ── Insert lot (lot_code = batch_number for the multi-species station) ────
  ...
  return NextResponse.json({ ok: true, batch_number: batchNumber, lot_id: lotId, qc_pass: qcPass, ccp_warnings: ccpWarnings });
}
