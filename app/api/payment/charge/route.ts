...
export async function POST(req: NextRequest) {
  try {
    ...
    const { orderId, amount, cardToken, customerName } = body;

    if (!orderId || !amount || !cardToken) {
      return NextResponse.json(
        { error: 'Missing required fields: orderId, amount, cardToken' },
        { status: 400 }
      );
    }
    ...
    const result =
      RBC_CONFIG.apiKey && RBC_CONFIG.gatewayUrl
...
async function simulateCharge(
  ...
  return {
    ...
    refNumber,
    approvalCode: isDeclined
      ? undefined
      : `APR${Math.floor(Math.random() * 999999)}`,
    message: isDeclined
      ? 'Card declined — simulation mode'
      : 'Payment approved — simulation mode',
    amount,
    currency: 'BSD',
  };
}
