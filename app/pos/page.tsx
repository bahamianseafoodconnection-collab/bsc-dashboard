        ...
        <div style={card}>
          ...
          {customerName && customerPhone ? (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#0a1f0a', borderRadius: 10, padding: '10px 12px', border: '1px solid #4ade80' }}>
              ...
              <button onClick={() => { setCustomerName(''); setCustomerPhone(''); setCustomerQuery(''); setSuggestions([]); }} style={{ background: 'none', border: 'none', color: '#6b7280', cursor: 'pointer', fontSize: 12 }}>
                Change
              </button>
            </div>
          ) : (
            ...
            <div key={product.id} style={card}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
                ...
                {inCart ? (
                  ...
                  <button onClick={() => addToCart(product)} style={{ padding: '10px 18px', borderRadius: 10, backgroundColor: '#f5c518', color: '#000', fontWeight: 'bold', border: 'none', cursor: 'pointer', fontSize: 14, flexShrink: 0 }}>
                    Add
                  </button>
                )}
              </div>
    ...
    <POSLayout showWaPanel={showWaPanel} setShowWaPanel={setShowWaPanel}>
      <div style={pg}>
          ...
          <div style={card}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' as const }}>
              {[cartTotal, Math.ceil(cartTotal / 5) * 5, Math.ceil(cartTotal / 10) * 10, Math.ceil(cartTotal / 20) * 20, 50, 100]
                ...
                <button key={amt} onClick={() => setCashGiven(amt.toFixed(2))} style={{ padding: '8px 14px', borderRadius: 8, backgroundColor: cashNum === amt ? '#f5c518' : '#1e3a5f', color: cashNum === amt ? '#000' : '#fff', border: 'none', cursor: 'pointer', fontWeight: 'bold', fontSize: 13 }}>
                  ${amt.toFixed(0)}
                </button>
              ))}
            </div>
        ...
        <button onClick={handleCompleteSale} disabled={processing || !paymentMethod || (paymentMethod === 'cash' && cashNum < cartTotal)} style={{
          ...
          backgroundColor: processing ? '#555' : (!paymentMethod || (paymentMethod === 'cash' && cashNum < cartTotal)) ? '#2a2a2a' : '#f5c518',
