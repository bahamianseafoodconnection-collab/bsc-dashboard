...
export default function POSPage() {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        {cartCount > 0 && (
          <button onClick={() => setScreen('cart')} style={{
            ...
            fontSize: 14, cursor: 'pointer', position: 'relative' as const,
          }}>
    ...
    <div style={pg}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
        <button
          ...
          style={{
            ...
            backgroundColor: paymentMethod === 'cash' ? '#f5c518' : '#0d1f3c',
            color: paymentMethod === 'cash' ? '#000' : '#aaa',
            border: paymentMethod === 'cash' ? 'none' : '1px solid #1e3a5f',
        ...
        <button
          ...
          style={{
            padding: '20px 16px', borderRadius: 14,
            backgroundColor: paymentMethod === 'card' ? '#f5c518' : '#0d1f3c',
            color: paymentMethod === 'card' ? '#000' : '#aaa',
            border: paymentMethod === 'card' ? 'none' : '1px solid #1e3a5f',
            fontWeight: 'bold', fontSize: 16, cursor: 'pointer',
            display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6,
          }}
        >
      ...
      {paymentMethod === 'cash' && (
        <div style={card}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' as const }}>
            {[cartTotal, Math.ceil(cartTotal / 5) * 5, Math.ceil(cartTotal / 10) * 10, Math.ceil(cartTotal / 20) * 20, Math.ceil(cartTotal / 50) * 50, 100].filter((v, i, a) => a.indexOf(v) === i).slice(0, 5).map(amt => (
              <button
                key={amt}
      ...
      {paymentMethod === 'card' && (
        <div style={{ ...card, textAlign: 'center', padding: 20 }}>
          ...
          <p style={{ margin: 0, color: '#aaa', fontSize: 14 }}>Process card payment on your terminal</p>
          <p style={{ margin: '6px 0 0', color: '#4a5568', fontSize: 12 }}>Tap Complete Sale when payment is confirmed</p>
        </div>
      )}
  ...
  if (screen === 'complete' && completedInvoice) return (
    <div style={pg}>
      ...
      {/* RECEIPT SUMMARY */}
      <div style={{ ...card, backgroundColor: '#fff', color: '#111', fontFamily: 'monospace' }}>
        <div style={{ textAlign: 'center', marginBottom: 14, paddingBottom: 10, borderBottom: '1px dashed #ccc' }}>
      ...
      {/* DELIVERY OPTIONS */}
      <p style={{ color: '#6b7280', fontSize: 11, letterSpacing: 1, margin: '20px 0 10px' }}>SEND INVOICE TO CUSTOMER</p>
      ...
      <button
        ...
        style={{ ...secondaryBtn, color: '#f5c518', borderColor: '#f5c518' }}
      >
        📄 View Full Invoice
      </button>
