            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: '10px' }}>
              {filtered.map((product) => (
                <button
                  ...
                  style={{ backgroundColor: '#fff', border: '1.5px solid #ebebeb', borderRadius: '14px', padding: '14px 10px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', cursor: 'pointer', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}
                >
            ...
            {cart.map((item) => (
              <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 0', borderBottom: '1px solid #f5f5f5' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ color: '#1a2e5a', fontWeight: 700, fontSize: '12px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {item.name}
                  </div>
                  <div style={{ color: '#999', fontSize: '11px' }}>${item.price.toFixed(2)} each</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <button onClick={() => changeQty(item.id, -1)} style={{ width: '24px', height: '24px', borderRadius: '6px', border: '1px solid #e5e7eb', backgroundColor: '#fff', cursor: 'pointer', fontWeight: 900, fontSize: '14px' }}>
                    -
                  </button>
                  <span style={{ fontWeight: 800, fontSize: '13px', minWidth: '18px', textAlign: 'center' }}>{item.qty}</span>
                  <button onClick={() => changeQty(item.id, 1)} style={{ width: '24px', height: '24px', borderRadius: '6px', border: 'none', backgroundColor: '#1a2e5a', color: '#fff', cursor: 'pointer', fontWeight: 900, fontSize: '14px' }}>
                    +
                  </button>
                </div>
                <div style={{ minWidth: '48px', textAlign: 'right' }}>
                  ...
                  <button onClick={() => removeItem(item.id)} style={{ background: 'none', border: 'none', color: '#ef4444', fontSize: '10px', cursor: 'pointer', padding: 0 }}>
                    remove
                  </button>
                </div>
              </div>
        ...
        <>
          ...
          <div style={{ position: 'fixed', right: 0, top: 0, bottom: 0, width: '320px', backgroundColor: '#fff', zIndex: 51, boxShadow: '-4px 0 24px rgba(0,0,0,0.15)', display: 'flex', flexDirection: 'column' }}>
            <div style={{ backgroundColor: '#25D366', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <button onClick={() => setWhatsappOpen(false)} style={{ background: 'none', border: 'none', color: '#fff', fontSize: '24px', cursor: 'pointer', lineHeight: 1 }}>
                ×
              </button>
            </div>
      ...
      {receiptVisible && lastSale && (
        <>
          ...
          <div style={{ position: 'fixed', inset: '20px', maxWidth: '400px', margin: '0 auto', backgroundColor: '#fff', borderRadius: '20px', zIndex: 61, overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ backgroundColor: '#1a2e5a', padding: '24px', textAlign: 'center', borderRadius: '20px 20px 0 0' }}>
            ...
            <div style={{ padding: '24px' }}>
              <div style={{ marginBottom: '16px' }}>
                <div style={{ color: '#999', fontSize: '12px', marginBottom: '2px' }}>Customer</div>
              ...
              <div style={{ marginTop: '16px', padding: '14px', backgroundColor: '#fef9e7', borderRadius: '12px', marginBottom: '16px' }}>
