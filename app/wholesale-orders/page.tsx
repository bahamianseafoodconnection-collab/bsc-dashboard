...
if (activeWholesaler !== 'all') {
query = query.eq('wholesaler', activeWholesaler);
}
if (activeFilter === 'pending') {
query = query.eq('admin_purchased', false);
} else if (activeFilter === 'purchased') {
query = query.eq('admin_purchased', true);
}
...
const wholesalerInfo = (key: string) => WHOLESALERS.find(w => w.key === key) || { name: key, color: '#1a2e5a', logo: '🏪' };
...
<div style={{ backgroundColor: '#f8f9fa', borderRadius: 10, padding: '10px 14px' }}>
<div style={{ color: '#999', fontSize: 10, marginBottom: 2 }}>Customer</div>
<div style={{ color: '#1a2e5a', fontWeight: 700, fontSize: 13 }}>{order.customer_name || 'Unknown'}</div>
</div>
<div style={{ backgroundColor: '#f8f9fa', borderRadius: 10, padding: '10px 14px' }}>
<div style={{ color: '#999', fontSize: 10, marginBottom: 2 }}>Phone</div>
<div style={{ color: '#1a2e5a', fontWeight: 700, fontSize: 13 }}>{order.customer_phone || '—'}</div>
</div>
<div style={{ backgroundColor: '#f8f9fa', borderRadius: 10, padding: '10px 14px' }}>
<div style={{ color: '#999', fontSize: 10, marginBottom: 2 }}>Delivery Address</div>
<div style={{ color: '#1a2e5a', fontWeight: 700, fontSize: 13 }}>{order.customer_address || '—'}</div>
</div>
<div style={{ backgroundColor: '#f8f9fa', borderRadius: 10, padding: '10px 14px' }}>
<div style={{ color: '#999', fontSize: 10, marginBottom: 2 }}>Payment</div>
<div style={{ color: '#1a2e5a', fontWeight: 700, fontSize: 13, textTransform: 'capitalize' }}>{order.payment_method || 'COD'}</div>
</div>
</div>
