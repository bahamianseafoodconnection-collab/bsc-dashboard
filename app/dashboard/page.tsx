...
items?: { name: string; price: number; quantity: number; cost?: number }[];
channel?: string;
supplier_id?: string;
...
type InvoiceSplit = {
wholesale: { item: string; qty: string; price: string }[];
retail: { item: string; qty: string; price: string }[];
supplierOwed: string;
bscKeeps: string;
summary: string;
} | null;
...
const [invoiceSplit, setInvoiceSplit] = useState<InvoiceSplit>(null);
...
setInvoiceSplit(null);
try {
const base64 = invoicePhoto.split(',')[1];
...
setInvoiceSplit(data.split || null);
} catch {
setInvoiceSplit(null);
}
...
<div style={{ color: '#f4c842', fontWeight: 900, fontSize: '20px' }}>{fmtBSD(todayRevenue)}</div>
</div>
...
<div style={{ color: '#2e7d32', fontWeight: 900, fontSize: '20px' }}>{fmtBSD(todayProfit)}</div>
</div>
...
<div style={{ color: '#dc2626', fontWeight: 900, fontSize: '20px' }}>{fmtBSD(todaySupplier)}</div>
</div>
</div>
...
{/* INVOICE SCANNER */}
<div style={{ backgroundColor: '#fff', borderRadius: '16px', padding: '18px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
<h2 style={{ color: '#1a2e5a', fontWeight: 900, fontSize: '15px', marginBottom: '6px' }}>📷 Invoice Scanner</h2>
<p style={{ color: '#999', fontSize: '12px', marginBottom: '14px' }}>Take a photo of any supplier invoice — AI will split wholesale vs retail and calculate what BSC owes the supplier.</p>
...
style={{ flex: 1, backgroundColor: '#1a2e5a', color: '#f4c842', border: 'none', borderRadius: '12px', padding: '14px', fontWeight: 800, fontSize: '14px', cursor: 'pointer' }}
>
📸 Take Photo
</button>
<button
onClick={() => { if (fileInputRef.current) { fileInputRef.current.removeAttribute('capture'); fileInputRef.current.click(); } }}
style={{ flex: 1, backgroundColor: '#f0f4ff', color: '#1a2e5a', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '14px', fontWeight: 800, fontSize: '14px', cursor: 'pointer' }}
>
📁 Upload File
</button>
</div>
...
<img src={invoicePhoto} alt="Invoice" style={{ width: '100%', borderRadius: '10px', border: '2px dashed #1a2e5a', maxHeight: '240px', objectFit: 'contain', backgroundColor: '#f8f9fa' }} />
<button
onClick={() => { setInvoicePhoto(null); setInvoiceSplit(null); }}
style={{ position: 'absolute', top: '8px', right: '8px', backgroundColor: '#dc2626', color: '#fff', border: 'none', borderRadius: '8px', padding: '4px 10px', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }}
>
Remove
</button>
</div>
...
style={{ width: '100%', backgroundColor: invoiceLoading ? '#94a3b8' : '#2e7d32', color: '#fff', border: 'none', borderRadius: '12px', padding: '14px', fontWeight: 800, fontSize: '14px', cursor: invoiceLoading ? 'not-allowed' : 'pointer' }}
>
{invoiceLoading ? 'Reading Invoice...' : '🤖 Analyze & Split Invoice'}
</button>
</div>
)}

{/* INVOICE SPLIT RESULTS */}
{invoiceSplit && (
<div style={{ marginTop: '16px' }}>
<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '14px' }}>
<div style={{ backgroundColor: '#e8f5e9', borderRadius: '12px', padding: '14px', textAlign: 'center' }}>
<div style={{ color: '#666', fontSize: '11px', marginBottom: '4px' }}>BSC Keeps</div>
<div style={{ color: '#2e7d32', fontWeight: 900, fontSize: '20px' }}>{invoiceSplit.bscKeeps}</div>
</div>
<div style={{ backgroundColor: '#fde8e8', borderRadius: '12px', padding: '14px', textAlign: 'center' }}>
<div style={{ color: '#666', fontSize: '11px', marginBottom: '4px' }}>Supplier Owed</div>
<div style={{ color: '#dc2626', fontWeight: 900, fontSize: '20px' }}>{invoiceSplit.supplierOwed}</div>
</div>
</div>

{invoiceSplit.wholesale.length > 0 && (
<div style={{ marginBottom: '12px' }}>
<div style={{ color: '#1a2e5a', fontWeight: 800, fontSize: '13px', marginBottom: '8px' }}>📦 Wholesale Items</div>
{invoiceSplit.wholesale.map((item, i) => (
<div key={i} style={{ display: 'flex', justifyContent: 'space-between', backgroundColor: '#f0fde8', borderRadius: '8px', padding: '8px 12px', marginBottom: '4px' }}>
<span style={{ color: '#1a2e5a', fontSize: '13px' }}>{item.item}</span>
<span style={{ color: '#666', fontSize: '12px' }}>{item.qty} · {item.price}</span>
</div>
))}
</div>
)}

{invoiceSplit.retail.length > 0 && (
<div style={{ marginBottom: '12px' }}>
<div style={{ color: '#1a2e5a', fontWeight: 800, fontSize: '13px', marginBottom: '8px' }}>🛒 Retail Items</div>
{invoiceSplit.retail.map((item, i) => (
<div key={i} style={{ display: 'flex', justifyContent: 'space-between', backgroundColor: '#e8f4fd', borderRadius: '8px', padding: '8px 12px', marginBottom: '4px' }}>
<span style={{ color: '#1a2e5a', fontSize: '13px' }}>{item.item}</span>
<span style={{ color: '#666', fontSize: '12px' }}>{item.qty} · {item.price}</span>
</div>
))}
...
<div style={{ backgroundColor: '#f8f9fa', borderRadius: '10px', padding: '12px', borderLeft: '4px solid #f4c842' }}>
<div style={{ color: '#1a2e5a', fontSize: '13px', lineHeight: 1.6 }}>{invoiceSplit.summary}</div>
</div>
</div>
)}
</div>
