...
export default function MarketPage() {
  ...
  async function checkSession() {
    try {
      ...
      if (u) {
        // Block staff from customer market
  ...
  async function handlePlaceOrder() {
    ...
    const { error } = await supabase.from('orders').insert({
      ...
      delivery_address: fulfillment === 'delivery' ? address + ', ' + island + (mailboat ? ' via ' + mailboat : '') : 'Pickup: ' + pickupDate,
  ...
  const allProducts = [
    ...
    ...supplierProducts.map(sp => ({
      id: sp.id, name: sp.name, price: sp.retail_price / markup,
      stock: 999, minStock: 0, category: sp.category,
      supplierName: sp.supplier_name, image: sp.photo_url || 'https://images.unsplash.com/photo-1534482421-64566f976cfa?w=400&q=80',
    ...
    <div style={{ background: 'linear-gradient(135deg, #060d1f, #0d1f3c)', borderBottom: '1px solid #1e3a5f', padding: '14px 18px', position: 'sticky' as const, top: 0, zIndex: 50 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', maxWidth: 640, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {cartCount > 0 && (
            <button onClick={() => setView('cart')} style={{ padding: '8px 14px', borderRadius: 20, backgroundColor: '#f5c518', color: '#000', fontWeight: 'bold', border: 'none', cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
              🛒 {cartCount}
            </button>
          )}
          {user ? (
            <button onClick={() => { setView('profile'); }} style={{ width: 38, height: 38, borderRadius: '50%', backgroundColor: '#0d1f3c', border: '2px solid #f5c518', cursor: 'pointer', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              👤
            </button>
          ) : (
            <button onClick={() => setView('login')} style={{ padding: '8px 14px', borderRadius: 20, backgroundColor: '#0d1f3c', color: '#f5c518', border: '1px solid #f5c518', cursor: 'pointer', fontSize: 13, fontWeight: 'bold' }}>
              Login
            </button>
          )}
        </div>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '0 0 20px' }}>
        ...
        {user && (
          <div style={{ margin: '16px 18px 0', backgroundColor: '#0a1f0a', border: '1px solid #4ade80', borderRadius: 12, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={() => { setView('orders'); loadMyOrders(); }} style={{ padding: '7px 12px', borderRadius: 10, backgroundColor: '#0d2b14', color: '#4ade80', border: '1px solid #4ade80', cursor: 'pointer', fontSize: 12, fontWeight: 'bold' }}>
              My Orders
            </button>
          </div>
        )}
        ...
        <div style={{ padding: '20px 18px 0' }}>
          ...
          <button onClick={() => { setActiveCategory('all'); setView('shop'); }} style={{ ...primaryBtn }}>
            View All Products →
          </button>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginTop: 8 }}>
            {[
              { icon: '🚚', title: 'Island Delivery', desc: 'All Bahamas islands' },
              { icon: '❄️', title: 'Fresh & Frozen', desc: 'Cold chain guaranteed' },
              { icon: '🐟', title: 'Local & Fresh', desc: 'Caught locally' },
            ].map(info => (
          ...
          {allProducts.map(product => {
            ...
            return (
              <div key={product.id} style={{ backgroundColor: '#0d1f3c', borderRadius: 16, overflow: 'hidden', border: '1px solid #1e3a5f' }}>
                ...
                <div style={{ padding: '10px 12px' }}>
                  <p style={{ margin: '0 0 2px', fontWeight: 'bold', fontSize: 13 }}>{product.name}</p>
                  <p style={{ margin: '0 0 8px', color: '#4a5568', fontSize: 10, lineHeight: 1.4 }}>
                    {(product as any).description?.slice(0, 50)}...
                  </p>
                  {inCart ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
  ...
  if (view === 'login') return (
    <div style={pg}>
      ...
      <div style={{ maxWidth: 420, margin: '0 auto', padding: '30px 18px' }}>
        ...
        <button onClick={() => { setAuthError(''); setView('register'); }} style={ghostBtn}>
  ...
  if (view === 'register') return (
    <div style={pg}>
      ...
      <div style={{ maxWidth: 420, margin: '0 auto', padding: '30px 18px' }}>
        ...
        {authError && <p style={{ color: '#f87171', fontSize: 13, backgroundColor: '#2d0000', padding: '10px 12px', borderRadius: 8, marginBottom: 12 }}>{authError}</p>}
        <button onClick={handleRegister} disabled={loading} style={{ ...primaryBtn, backgroundColor: loading ? '#555' : '#f5c518', cursor: loading ? 'not-allowed' : 'pointer' }}>
    ...
    <div style={pg}>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '16px 18px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
          <button onClick={() => setFulfillment('delivery')} style={{ flex: 1, padding: '14px', borderRadius: 12, backgroundColor: fulfillment === 'delivery' ? '#f5c518' : '#0d1f3c', color: fulfillment === 'delivery' ? '#000' : '#aaa', border: fulfillment === 'delivery' ? 'none' : '1px solid #1e3a5f', fontWeight: 'bold', fontSize: 14, cursor: 'pointer' }}>
            🚚 Delivery +$15
          </button>
          <button onClick={() => setFulfillment('pickup')} style={{ flex: 1, padding: '14px', borderRadius: 12, backgroundColor: fulfillment === 'pickup' ? '#f5c518' : '#0d1f3c', color: fulfillment === 'pickup' ? '#000' : '#aaa', border: fulfillment === 'pickup' ? 'none' : '1px solid #1e3a5f', fontWeight: 'bold', fontSize: 14, cursor: 'pointer' }}>
            🏪 Pickup FREE
          </button>
        </div>
            ...
            {isOutIsland && availableMailboats.length > 0 && (
              <>
                ...
                <div style={{ backgroundColor: '#1a1400', border: '1px solid #f5c518', borderRadius: 10, padding: '10px 14px', marginBottom: 12 }}>
                  <p style={{ margin: 0, color: '#f5c518', fontSize: 12 }}>⚠️ Orders must be placed 48 hours before mailboat departure</p>
                </div>
              </>
            )}
            ...
            <div key={order.id} style={{ backgroundColor: '#0d1f3c', borderRadius: 16, padding: '16px', border: '1px solid #1e3a5f', marginBottom: 14 }}>
              ...
              <div style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  {['pending', 'confirmed', 'packing', 'out_for_delivery', 'delivered'].map((s, i) => {
                    const statuses = ['pending', 'confirmed', 'packing', 'out_for_delivery', 'delivered'];
                    const currentIdx = statuses.indexOf(order.status);
                    const isDone = i <= currentIdx;
                    return (
                      <div key={s} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        <div style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: isDone ? '#4ade80' : '#1e3a5f', marginBottom: 4 }} />
                        {i < 4 && <div style={{ height: 2, backgroundColor: isDone && i < currentIdx ? '#4ade80' : '#1e3a5f', position: 'absolute', width: '100%' }} />}
                      </div>
                    );
                  })}
                </div>
                <div style={{ height: 3, backgroundColor: '#1e3a5f', borderRadius: 3, overflow: 'hidden' }}>
                  <div style={{
                    ...
                    width: order.status === 'pending' ? '10%' : order.status === 'confirmed' ? '30%' : order.status === 'packing' ? '55%' : order.status === 'out_for_delivery' ? '80%' : '100%',
        ...
        {!user ? (
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
              <div style={{ backgroundColor: '#0d1f3c', borderRadius: 12, padding: '14px', border: '1px solid #1e3a5f', textAlign: 'center' as const }}>
                <p style={{ margin: '0 0 4px', color: '#f5c518', fontWeight: 'bold', fontSize: 22 }}>{myOrders.length}</p>
                <p style={{ margin: 0, color: '#4a5568', fontSize: 12 }}>Total Orders</p>
              </div>
              <div style={{ backgroundColor: '#0d1f3c', borderRadius: 12, padding: '14px', border: '1px solid #1e3a5f', textAlign: 'center' as const }}>
                <p style={{ margin: '0 0 4px', color: '#4ade80', fontWeight: 'bold', fontSize: 22 }}>${myOrders.reduce((s, o) => s + Number(o.total), 0).toFixed(0)}</p>
                <p style={{ margin: 0, color: '#4a5568', fontSize: 12 }}>Total Spent</p>
              </div>
            </div>
            ...
            <div style={{ backgroundColor: '#0d1f3c', borderRadius: 16, padding: '16px', border: '1px solid #1e3a5f', marginBottom: 16 }}>
              ...
              <button onClick={handleProfileSave} disabled={loading} style={{ ...primaryBtn, marginBottom: 0 }}>
                {loading ? 'Saving...' : 'Save Changes'}
              </button>
            </div>

            <button onClick={() => { setView('orders'); loadMyOrders(); }} style={{ ...ghostBtn }}>
              📦 View My Orders
            </button>

            <button onClick={async () => { await supabase.auth.signOut(); setUser(null); setMyOrders([]); setView('home'); }} style={{ ...ghostBtn, color: '#f87171', borderColor: '#f87171' }}>
              Sign Out
            </button>
          </>
        )}
