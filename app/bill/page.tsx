...
const [tab, setTab] = useState<'overview' | 'inventory' | 'yield' | 'profit'>('overview');
...
<p style={{ margin: 0, color: '#4a5568', fontSize: 11 }}>Tropic Seafood · Beaver Street · Partner</p>
</div>
...
{ label: 'SOLD VALUE', value: '$' + totalSales.toFixed(0), color: '#4ade80' },
].map(k => (
<div key={k.label} style={{ ...card, textAlign: 'center', marginBottom: 0 }}>
<p style={{ margin: 0, color: '#4a5568', fontSize: 9 }}>{k.label}</p>
...
<button key={t.key} onClick={() => setTab(t.key as any)} style={{ flex: 1, padding: '9px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 'bold', backgroundColor: tab === t.key ? '#f5c518' : '#0d1f3c', color: tab === t.key ? '#000' : '#6b7280' }}>
{t.label}
</button>
))}
</div>
...
{[
{ icon: '🐟', task: 'Manage Tropic Seafood sales & purchases' },
...
{ icon: '💰', task: '5% partnership share of BSC total profit' },
].map((r, i) => (
...
{products.length === 0 ? <div style={{ ...card, textAlign: 'center', padding: 24 }}><p style={{ color: '#4a5568' }}>No products yet. Upload via Supplier Portal.</p></div> : products.map(p => (
<div key={p.id} style={card}>
<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
<div>
{p.photo_url && <img src={p.photo_url} alt={p.name} style={{ width: 48, height: 48, borderRadius: 8, objectFit: 'cover', marginBottom: 8 }} />}
<p style={{ margin: 0, fontWeight: 'bold', fontSize: 14 }}>{p.name}</p>
<p style={{ margin: '2px 0', color: '#4a5568', fontSize: 11 }}>{p.category}</p>
</div>
<div style={{ textAlign: 'right' as const }}>
<p style={{ margin: 0, color: '#4ade80', fontWeight: 'bold', fontSize: 16 }}>${p.retail_price?.toFixed(2)}</p>
<p style={{ margin: '2px 0', color: p.stock_qty <= 5 ? '#f87171' : '#4a5568', fontSize: 12 }}>{p.stock_qty || 0} in stock</p>
<span style={{ backgroundColor: p.status === 'approved' ? '#0a1f0a' : '#1a1400', color: p.status === 'approved' ? '#4ade80' : '#f5c518', borderRadius: 20, padding: '2px 8px', fontSize: 10, fontWeight: 'bold' }}>{p.status.toUpperCase()}</span>
</div>
</div>
</div>
))}
...
{batches.length === 0 ? <div style={{ ...card, textAlign: 'center', padding: 24 }}><p style={{ color: '#4a5568' }}>No batches recorded yet.</p></div> : batches.map(b => (
<div key={b.id} style={card}>
<p style={{ margin: 0, color: '#f5c518', fontSize: 12, fontFamily: 'monospace' }}>{b.batch_number}</p>
<p style={{ margin: '4px 0', fontWeight: 'bold', fontSize: 14 }}>{b.product_name}</p>
<p style={{ margin: '2px 0', color: '#4a5568', fontSize: 12 }}>Producer: {b.producer_name}</p>
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginTop: 10 }}>
{[
{ label: 'Yield', value: b.yield_pct + '%', color: '#4ade80' },
{ label: 'Out', value: b.weight_out_lbs + ' lbs', color: '#60a5fa' },
{ label: 'Cost/lb', value: '$' + b.cost_per_lb_processed?.toFixed(2), color: '#f5c518' },
].map(x => (
<div key={x.label} style={{ backgroundColor: '#060d1f', borderRadius: 8, padding: '7px 10px' }}>
<p style={{ margin: 0, color: '#4a5568', fontSize: 9 }}>{x.label}</p>
...
<p style={{ margin: '0 0 16px', color: '#4a5568', fontSize: 12 }}>Partner share is calculated monthly from total BSC gross profit.</p>
<div style={{ backgroundColor: 'rgba(0,0,0,0.3)', borderRadius: 10, padding: '14px 16px' }}>
<p style={{ margin: '0 0 6px', color: '#4a5568', fontSize: 11' }}>Total product sales value tracked</p>
<p style={{ margin: 0, color: '#4ade80', fontWeight: 'bold', fontSize: 24 }}>${totalSales.toFixed(2)}</p>
<p style={{ margin: '8px 0 0', color: '#4a5568', fontSize: 11 }}>Full profit share calculated by Dedrick monthly from BSC Control Dashboard.</p>
</div>
</div>
)}
</div>
