...
// Card / Bank Transfer → Auto-approved → Fulfillment starts immediately
// Cash on Delivery     → HOLD → BSC must reconfirm with customer first
//                        Prevents wasted delivery + customer no-shows
// ────────────────────────────────────────────────────────────
type PaymentMethod = 'card' | 'bank_transfer' | 'paypal' | 'cash_on_delivery';
...
function isCOD(method: PaymentMethod): boolean {
  return method === 'cash_on_delivery';
}

function canFulfill(method: PaymentMethod, codConfirmed: boolean): boolean {
  if (isCOD(method)) return codConfirmed;
  return true; // card, bank transfer, paypal auto-approved
}
...
function printCustomerInvoice(order: Order) {
  ...
  const isCashOrder   = isCOD(order.paymentMethod);
  const fulfillReady  = canFulfill(order.paymentMethod, order.codConfirmed);
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f8f9fa', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ maxWidth: '700px', margin: '0 auto', padding: '20px 16px' }}>
        ...
        {isCashOrder && !order.codConfirmed && (
          <div style={{ backgroundColor: '#fff8e1', border: '2px solid #f4c842', borderRadius: '16px', padding: '18px', marginBottom: '14px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
              <span style={{ fontSize: '28px' }}>⚠️</span>
              <div>
                <div style={{ color: '#1a2e5a', fontWeight: 900, fontSize: '15px' }}>Cash on Delivery — Awaiting BSC Reconfirmation</div>
                <div style={{ color: '#666', fontSize: '12px', marginTop: '2px' }}>Do NOT pack or dispatch until customer confirms. Prevents wasted delivery trips.</div>
              </div>
            </div>
            <div style={{ backgroundColor: '#fef9e7', borderRadius: '10px', padding: '12px 14px', marginBottom: '12px' }}>
              <div style={{ color: '#d97706', fontSize: '12px', fontWeight: 700, marginBottom: '4px' }}>⏸️ FULFILLMENT ON HOLD</div>
              <div style={{ color: '#666', fontSize: '11px', lineHeight: 1.6 }}>
                1. Contact customer to reconfirm order and delivery address<br/>
                2. Confirm customer will be available to receive and pay cash<br/>
                3. Only after verbal confirmation — mark as confirmed below<br/>
                4. Then proceed to pack and dispatch
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              <a
                href={`https://wa.me/${order.customer.phone.replace(/\D/g,'')}?text=Hi ${order.customer.name}! Your BSC Marketplace order ${order.id} is ready. Total: $${order.total.toFixed(2)} Cash on Delivery. Please confirm you will be available to receive and pay. Reply YES to confirm.`}
                target="_blank"
                rel="noreferrer"
                style={{ display: 'block', backgroundColor: '#25D366', color: '#fff', textDecoration: 'none', borderRadius: '10px', padding: '12px', textAlign: 'center', fontWeight: 800, fontSize: '13px' }}
              >
                💬 WhatsApp Customer
              </a>
              <button
                onClick={() => setCodConfirming(true)}
                style={{ backgroundColor: '#1a2e5a', color: '#f4c842', border: 'none', borderRadius: '10px', padding: '12px', fontWeight: 800, fontSize: '13px', cursor: 'pointer' }}
              >
                ✅ Mark as Confirmed
              </button>
            </div>
        ...
        {codConfirming && (
          <>
            <div onClick={() => setCodConfirming(false)} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 60 }} />
            <div style={{ position: 'fixed', inset: '20px', maxWidth: '400px', margin: '0 auto', backgroundColor: '#fff', borderRadius: '20px', zIndex: 61, padding: '28px', boxShadow: '0 24px 64px rgba(0,0,0,0.3)', height: 'fit-content', top: '50%', transform: 'translateY(-50%)' }}>
              <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                <div style={{ fontSize: '48px', marginBottom: '10px' }}>📞</div>
                <div style={{ color: '#1a2e5a', fontWeight: 900, fontSize: '18px' }}>Confirm COD Reconfirmation</div>
                <div style={{ color: '#666', fontSize: '13px', marginTop: '6px' }}>Only confirm if you have spoken with the customer and they have verbally agreed to receive and pay cash.</div>
              </div>
              <div style={{ backgroundColor: '#f8f9fa', borderRadius: '12px', padding: '14px', marginBottom: '18px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                  <span style={{ color: '#999', fontSize: '12px' }}>Customer</span>
                  <span style={{ color: '#1a2e5a', fontWeight: 700, fontSize: '12px' }}>{order.customer.name}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                  <span style={{ color: '#999', fontSize: '12px' }}>Phone</span>
                  <span style={{ color: '#1a2e5a', fontWeight: 700, fontSize: '12px' }}>{order.customer.phone}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#999', fontSize: '12px' }}>Amount Due</span>
                  <span style={{ color: '#1a2e5a', fontWeight: 900, fontSize: '14px' }}>${order.total.toFixed(2)}</span>
                </div>
              </div>
              <button
                onClick={() => { setOrder((o) => ({ ...o, codConfirmed: true })); setCodConfirming(false); }}
                style={{ width: '100%', backgroundColor: '#2e7d32', color: '#fff', border: 'none', borderRadius: '12px', padding: '14px', fontWeight: 900, fontSize: '15px', cursor: 'pointer', marginBottom: '10px' }}
              >
                ✅ Yes — Customer Confirmed. Proceed to Pack
              </button>
              <button
                onClick={() => setCodConfirming(false)}
                style={{ width: '100%', backgroundColor: '#f8f9fa', color: '#666', border: '1.5px solid #e5e7eb', borderRadius: '12px', padding: '13px', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }}
              >
                Cancel
              </button>
            </div>
        ...
        {/* COD CONFIRMED BADGE */}
        {isCashOrder && order.codConfirmed && (
          <div style={{ backgroundColor: '#e8f5e9', border: '2px solid #2e7d32', borderRadius: '16px', padding: '14px 18px', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ fontSize: '24px' }}>✅</span>
            <div>
              <div style={{ color: '#2e7d32', fontWeight: 900, fontSize: '14px' }}>COD Reconfirmed — Ready to Fulfill</div>
              <div style={{ color: '#666', fontSize: '11px', marginTop: '2px' }}>Customer verbally confirmed. Safe to pack and dispatch.</div>
            </div>
          </div>
        )}
        ...
        <div style={{ backgroundColor: '#fff', borderRadius: '16px', padding: '18px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            ...
            {/* FULFILLMENT HOLD WARNING */}
            {isCashOrder && !order.codConfirmed && (
              <div style={{ backgroundColor: '#fef9e7', border: '1.5px solid #f4c842', borderRadius: '10px', padding: '12px 14px', textAlign: 'center', color: '#d97706', fontWeight: 700, fontSize: '13px' }}>
                ⏸️ Printing locked — Reconfirm COD with customer first
              </div>
            )}
            ...
            <button
              ...
              disabled={isCashOrder && !order.codConfirmed}
              style={{ backgroundColor: '#1a2e5a', color: '#f4c842', border: 'none', borderRadius: '12px', padding: '14px 18px', fontWeight: 900, fontSize: '14px', cursor: (isCashOrder && !order.codConfirmed) ? 'not-allowed' : 'pointer', opacity: (isCashOrder && !order.codConfirmed) ? 0.4 : 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
            >
